...
SEED = Path("/workspace/scripts/data/surfboard-catalog-seed/small-usa-aus-shapers-2026-08-25.json")
...
def download(url: str) -> tuple[bytes, str]:
    ...
    try:
        ...
        if exc.code not in {403, 401}:
            raise
        # Some hosts (WordPress / nginx WAFs) reject urllib but allow curl.
...
def mirror(key: str, source_url: str, kind: str) -> str | None:
    ...
    if kind == "logo":
        path = f"logos/daily-2026-08-25-{digest}.{ext}"
    else:
