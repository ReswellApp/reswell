...
SEED = Path("/workspace/scripts/data/surfboard-catalog-seed/small-usa-aus-shapers-2026-09-02.json")
...
def download(url: str) -> tuple[bytes, str]:
    ...
    try:
        ...
        if exc.code not in {403, 401}:
            raise
        dest = Path("/tmp/catalog-image-download.bin")
        ...
        if proc.returncode != 0:
            raise RuntimeError(proc.stderr.strip() or f"curl {proc.returncode}") from exc
        ctype = (proc.stdout or "image/jpeg").split(";")[0].strip() or "image/jpeg"
        raw = dest.read_bytes()
        if raw[:12] == b"\x89PNG\r\n\x1a\n\x00\x00\x00\r"[:8] or raw[:8] == b"\x89PNG\r\n\x1a\n":
...
def ext_for(url: str, ctype: str) -> str:
    ctype = ctype.lower()
    if "png" in ctype and "avif" not in ctype:
        return "png"
    if "webp" in ctype and "avif" not in ctype:
        return "webp"
    if "svg" in ctype:
        return "svg"
...
def mirror(key: str, source_url: str, kind: str) -> str | None:
    ...
    if kind == "logo":
        path = f"logos/daily-2026-09-02-{digest}.{ext}"
    else:
...
def main() -> None:
        ...
        json.dumps(
            {
                "seed": str(SEED),
                "brands": len(brands),
                "models": seed["summary"]["model_count"],
            },
            indent=2,
        )
    )
    ...
    for brand in brands:
        ...
        if brand.get("logo_url"):
            logo_url = mirror(key, brand["logo_url"], "logo")
            if not logo_url:
                errors.append(f"{slug}: logo mirror failed")
                print("FAIL logo", slug)
                continue
        ...
        if found:
            ...
            created_brands += 1
            inserted_brand_ids.append(brand_id)
        ...
        for model in brand["models"]:
            ...
            if not image_url:
                skipped_models += 1
                errors.append(f"{slug}/{name}: image mirror failed")
                print("  skip imageless", slug, name)
                continue
            row = {
                "brand_id": brand_id,
                "name": name,
            ...
            if model.get("board_category_slug"):
                row["board_category_slug"] = model["board_category_slug"]
        ...
        if brand_id in inserted_brand_ids and mirrored_ok == 0:
            print(f"DELETE imageless new brand {slug}")
            rest("DELETE", f"/rest/v1/brands?id=eq.{brand_id}", key)
            created_brands -= 1
            continue
