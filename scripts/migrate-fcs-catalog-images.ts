/**
 * Mirror FCS fin catalog images from external CDNs into Supabase `brand-assets`.
 *
 * Usage:
 *   npx tsx scripts/migrate-fcs-catalog-images.ts [--dry-run] [--force-csv] [csv-path]
 */
...
import {
  DEFAULT_FCS_FIN_CSV,
  loadFcsFinCsvByProductName,
  resolveFcsModelImageFromCsv,
  resolveFcsVariantImageFromCsv,
...
async function migrate(dryRun: boolean, csvPath: string, forceCsv: boolean): Promise<void> {
  ...
  const csvByName = loadFcsFinCsvByProductName(csvPath)

  const { data: models, error: modelsErr } = await supabase
    .from("brand_models")
    .select("id, name, image_url")
    .eq("brand_id", FCS_BRAND_ID)
    .order("name")
  ...
  for (const model of models ?? []) {
    ...
    if (csvRow && (!isValidHttpImageSource(current) || isExternalBrandCatalogImageUrl(current))) {
      const repaired = resolveFcsModelImageFromCsv(csvRow)
  ...
  for (const variant of variants ?? []) {
    ...
    if (csvRow && needsRepair) {
      const repaired =
        resolveFcsVariantImageFromCsv(csvRow, variant.fin_color_label ?? "") ?? null
  ...
  const { count: externalModelsLeft } = await supabase
    ...
    .eq("brand_id", FCS_BRAND_ID)
    .or("image_url.ilike.%shopify%,image_url.ilike.%surffcs%")
