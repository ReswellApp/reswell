/**
 * Bulk import Gato Heroi surfboard models from Thunderbit JSON export.
 * Imports model name and hero image only (no variants).
 *
 * Usage:
 *   npx tsx scripts/import-gato-heroi-catalog.ts [--dry-run] [json-path]
 */
...
import {
  DEFAULT_GATO_HEROI_JSON,
  GATO_HEROI_BRAND_SLUG,
  loadGatoHeroiJsonRows,
  resolveGatoHeroiModelImage,
} from "@/lib/services/gatoHeroiSurfboardCatalogJson"
...
async function resolveBrandId(supabase: SupabaseClient): Promise<string> {
  const { data, error } = await supabase
    .from("brands")
    .select("id")
    .eq("slug", GATO_HEROI_BRAND_SLUG)
    .maybeSingle()

  if (error || !data?.id) {
    throw new Error(`Brand not found for slug "${GATO_HEROI_BRAND_SLUG}"`)
  }
  return data.id
}
...
async function importCatalog(
  ...
  const rows = loadGatoHeroiJsonRows(jsonPath)

  console.log(
    JSON.stringify(
      {
        brandId,
        brandSlug: GATO_HEROI_BRAND_SLUG,
        jsonPath,
        dryRun,
  ...
  for (const row of rows) {
    ...
    const modelImageUrl = await resolveMirroredBrandCatalogImageUrl({
      ...
      supabase,
      supabaseUrl,
      sourceUrl: resolveGatoHeroiModelImage(row.productImage),
      kind: "model",
      logLabel: "import gato heroi",
    })
...
async function main(): Promise<void> {
  ...
  const jsonPath = jsonArg ? resolve(jsonArg) : DEFAULT_GATO_HEROI_JSON
