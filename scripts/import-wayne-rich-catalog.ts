/**
 * Bulk import Wayne Rich surfboard models from Thunderbit JSON export.
 * Imports model name and hero image only (no variants).
 *
 * Usage:
 *   npx tsx scripts/import-wayne-rich-catalog.ts [--dry-run] [json-path]
 */
...
import {
  DEFAULT_WAYNE_RICH_JSON,
  loadWayneRichJsonRows,
  resolveWayneRichModelImage,
  WAYNE_RICH_BRAND_SLUG,
} from "@/lib/services/wayneRichSurfboardCatalogJson"
...
async function resolveBrandId(supabase: SupabaseClient): Promise<string> {
  const { data, error } = await supabase
    .from("brands")
    .select("id")
    .eq("slug", WAYNE_RICH_BRAND_SLUG)
    .maybeSingle()

  if (error || !data?.id) {
    throw new Error(`Brand not found for slug "${WAYNE_RICH_BRAND_SLUG}"`)
  }
  return data.id
}
...
async function importCatalog(
  ...
  const rows = loadWayneRichJsonRows(jsonPath)

  console.log(
    JSON.stringify(
      {
        brandId,
        brandSlug: WAYNE_RICH_BRAND_SLUG,
        jsonPath,
        dryRun,
  ...
  for (const row of rows) {
    ...
    const modelImageUrl = await resolveMirroredBrandCatalogImageUrl({
      ...
      supabase,
      supabaseUrl,
      sourceUrl: resolveWayneRichModelImage(row.productImage),
      kind: "model",
      logLabel: "import wayne rich",
    })
...
async function main(): Promise<void> {
  ...
  const jsonPath = jsonArg ? resolve(jsonArg) : DEFAULT_WAYNE_RICH_JSON
