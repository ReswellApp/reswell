/**
 * Bulk import DHD Surfboards models from Thunderbit JSON export.
 * Imports model name and hero image only (no variants).
 *
 * Usage:
 *   npx tsx scripts/import-dhd-catalog.ts [--dry-run] [json-path]
 */
...
import {
  ...
  DHD_BRAND_SLUG,
  loadDhdJsonRows,
  resolveDhdModelImage,
} from "@/lib/services/dhdSurfboardCatalogJson"
...
async function resolveBrandId(supabase: SupabaseClient): Promise<string> {
  const { data, error } = await supabase
    .from("brands")
    .select("id")
    .eq("slug", DHD_BRAND_SLUG)
    .maybeSingle()

  if (error || !data?.id) {
    throw new Error(`Brand not found for slug "${DHD_BRAND_SLUG}"`)
  }
  return data.id
}
...
async function importCatalog(
  ...
  const rows = loadDhdJsonRows(jsonPath)

  console.log(
    JSON.stringify(
      {
        brandId,
        brandSlug: DHD_BRAND_SLUG,
        jsonPath,
        dryRun,
  ...
  for (const row of rows) {
    ...
    const modelImageUrl = await resolveMirroredBrandCatalogImageUrl({
      ...
      supabase,
      supabaseUrl,
      sourceUrl: resolveDhdModelImage(row.productImage),
      kind: "model",
      logLabel: "import dhd",
    })
...
async function main(): Promise<void> {
  ...
  const jsonPath = jsonArg ? resolve(jsonArg) : DEFAULT_DHD_JSON
