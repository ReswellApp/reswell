...
async function resolveMirroredImageUrl(
  ...
  const trimmed = preferFuturesProductImageUrl(sourceUrl?.trim() ?? "")
  ...
  if (!result.ok) {
    console.warn(`[import futures fins] image mirror failed (${kind}): ${result.error}`)
    return null
  }
