/**
 * Bulk import A&H Vessels surfboard models from Thunderbit JSON export.
 * Imports model name and hero image only (no variants).
 *
 * Usage:
 *   npx tsx scripts/import-ah-vessels-catalog.ts [--dry-run] [json-path]
 */
...
import {
  AH_VESSELS_BRAND_SLUG,
  DEFAULT_AH_VESSELS_JSON,
  loadAhVesselsJsonRows,
  resolveAhVesselsModelImage,
} from "@/lib/services/ahVesselsSurfboardCatalogJson"
...
async function resolveBrandId(supabase: SupabaseClient): Promise<string> {
  const { data, error } = await supabase
    .from("brands")
    .select("id")
    .eq("slug", AH_VESSELS_BRAND_SLUG)
    .maybeSingle()

  if (error || !data?.id) {
    throw new Error(`Brand not found for slug "${AH_VESSELS_BRAND_SLUG}"`)
  }
  return data.id
}
...
async function importCatalog(
  ...
  const rows = loadAhVesselsJsonRows(jsonPath)

  console.log(
    JSON.stringify(
      {
        brandId,
        brandSlug: AH_VESSELS_BRAND_SLUG,
        jsonPath,
        dryRun,
  ...
  for (const row of rows) {
    ...
    const modelImageUrl = await resolveMirroredBrandCatalogImageUrl({
      ...
      supabase,
      supabaseUrl,
      sourceUrl: resolveAhVesselsModelImage(row.productImage),
      kind: "model",
      logLabel: "import a&h vessels",
    })
...
async function main(): Promise<void> {
  ...
  const jsonPath = jsonArg ? resolve(jsonArg) : DEFAULT_AH_VESSELS_JSON
