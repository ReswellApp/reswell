...
export const APP_LLM_FEATURES: readonly AppLlmFeatureDefinition[] = [
  {
    ...
    purpose:
      "Turns marketplace searches into structured filters and ranks Elasticsearch title+catalog candidates so unlinked listings still match. Uses search synonyms and admin Good/Close/Bad memory. Retrieval stays in Elasticsearch; the LLM never invents listing ids.",
    ...
    surfaces: ["/boards", "/search", "GET /api/search/nl-helper"],
    sourceFiles: [
      "lib/services/marketplaceQueryUnderstand.ts",
      "lib/services/marketplaceNlHelper.ts",
      "lib/services/marketplaceListingMatch.ts",
