/**
 * Business logic for creating a accessory listing. A accessory is a single `listings`
 * row (section = 'accessories') that carries both the shared commerce fields and the
 * accessory-specific attributes (size, brand, model), with photos in
 * `listing_images`.
 *
...
import type {
  CreateAccessoryListingInput,
  UpdateAccessoryListingInput,
} from "@/lib/validations/accessory-listing"

export type CreateAccessoryListingResult = { listingId: string; slug: string }

export type AccessoryListingImageUpdateOp = {
  id?: string
  url: string
...
export async function syncAccessoryListingImages(
  supabase: SupabaseClient,
  listingId: string,
  removedImageIds: string[],
  images: AccessoryListingImageUpdateOp[],
): Promise<void> {
...
export async function updateAccessoryListing(
  supabase: SupabaseClient,
  listingId: string,
  userId: string,
  input: UpdateAccessoryListingInput,
): Promise<{ slug: string }> {
  ...
  if (existing.section !== ACCESSORIES_SECTION) {
    throw new Error("Only accessory listings can be edited here")
  }
  ...
  if (updateError || !updated) {
    throw new Error(updateError?.message ?? "Failed to update accessory listing")
  }

  const imageOps: AccessoryListingImageUpdateOp[] = input.images.map((img, index) => ({
    id: img.id,
    url: img.url,
  ...
  await syncAccessoryListingImages(supabase, listingId, input.removedImageIds ?? [], imageOps)

  return { slug: (updated.slug as string) ?? (existing.slug as string) }
}

export async function createAccessoryListing(
  supabase: SupabaseClient,
  userId: string,
  ...
  if (listingError || !inserted) {
    throw new Error(listingError?.message ?? "Failed to create accessory listing")
  }
