/**
 * Business logic for creating a traction listing. A traction is a single `listings`
 * row (section = 'traction') that carries both the shared commerce fields and the
 * traction-specific attributes (size, brand, model), with photos in
 * `listing_images`.
 *
...
import type {
  CreateTractionListingInput,
  UpdateTractionListingInput,
} from "@/lib/validations/traction-listing"

export type CreateTractionListingResult = { listingId: string; slug: string }

export type TractionListingImageUpdateOp = {
  id?: string
  url: string
...
export async function syncTractionListingImages(
  supabase: SupabaseClient,
  listingId: string,
  removedImageIds: string[],
  images: TractionListingImageUpdateOp[],
): Promise<void> {
...
export async function updateTractionListing(
  supabase: SupabaseClient,
  listingId: string,
  userId: string,
  input: UpdateTractionListingInput,
): Promise<{ slug: string }> {
  ...
  if (existing.section !== TRACTION_SECTION) {
    throw new Error("Only traction listings can be edited here")
  }
  if (existing.status === "sold") {
    throw new Error("Sold listings cannot be edited")
  }

  const updateFields = buildTractionListingPersistFields(input, {
  ...
  if (updateError || !updated) {
    throw new Error(updateError?.message ?? "Failed to update traction listing")
  }

  const imageOps: TractionListingImageUpdateOp[] = input.images.map((img, index) => ({
    id: img.id,
    url: img.url,
  ...
  await syncTractionListingImages(supabase, listingId, input.removedImageIds ?? [], imageOps)
...
export async function createTractionListing(
  supabase: SupabaseClient,
  userId: string,
  input: CreateTractionListingInput,
): Promise<CreateTractionListingResult> {
  const slug = await generateUniqueListingSlug(supabase, input.title)
  const persistFields = buildTractionListingPersistFields(input, {
    allowPrivilegedShippingModes: true,
  })
  ...
  if (listingError || !inserted) {
    throw new Error(listingError?.message ?? "Failed to create traction listing")
  }
