...
export async function fetchRecentVercelRequestLogs(input?: {
  ...
  if (!teamId) {
    throw new Error(
      "VERCEL_TEAM_ID is not set — copy your team id from Vercel → Team Settings → General",
    )
  }
...
function classifyLogEntry(entry: VercelRequestLogEntry): PlatformLogIssue | null {
  ...
  if (userFacing4xx && (path.startsWith("/api/") || path.startsWith("/checkout") || path.startsWith("/auth"))) {
    return {
      severity: "warning",
...
export function analyzeVercelRequestLogs(
  ...
  for (const entry of logs) {
    ...
    if (entry.id && existing.sampleRequestIds.length < 3 && !existing.sampleRequestIds.includes(entry.id)) {
...
export async function scanVercelPlatformLogs(input?: {
  ...
  if (!vercelAccessToken() || !vercelProjectId() || !vercelTeamId()) {
    return {
      ...
      issues: [],
      skippedReason:
        "Missing VERCEL_ACCESS_TOKEN, VERCEL_PROJECT_ID, or VERCEL_TEAM_ID",
    }
  }
...
export async function runVercelErrorDigest(
  ...
  const scan = await scanVercelPlatformLogs({ sinceHours: rangeHours, environment: "production" })
