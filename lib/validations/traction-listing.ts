...
export const TRACTION_LISTING_TITLE_MAX_LENGTH = 60
export const TRACTION_LISTING_MIN_PHOTOS = 1
export const TRACTION_LISTING_MAX_PHOTOS = 12

const tractionSizeValues = TRACTION_SIZE_OPTIONS.map((o) => o.value)
...
const tractionListingImageSchema = z.object({
...
const tractionListingBaseSchema = z.object({
  title: z.string().trim().min(3, "Add a title").max(TRACTION_LISTING_TITLE_MAX_LENGTH),
  ...
  size: optionalSlug(tractionSizeValues),
  ...
  images: z
    .array(tractionListingImageSchema)
    .min(TRACTION_LISTING_MIN_PHOTOS, "Add at least one photo")
    .max(TRACTION_LISTING_MAX_PHOTOS),
...
const tractionListingUpdateBaseSchema = tractionListingBaseSchema.extend({
  listingId: z.string().uuid(),
  removedImageIds: z.array(z.string().uuid()).optional().default([]),
  removedVideoIds: listingRemovedVideoIdsSchema,
})

export const createTractionListingSchema = withTractionListingRefinements(tractionListingBaseSchema)
...
export type CreateTractionListingInput = z.infer<typeof tractionListingBaseSchema>

export const updateTractionListingSchema = withTractionListingRefinements(tractionListingUpdateBaseSchema)

export type UpdateTractionListingInput = z.infer<typeof tractionListingUpdateBaseSchema>
