...
export const BOARDBAG_LISTING_TITLE_MAX_LENGTH = 60
export const BOARDBAG_LISTING_MIN_PHOTOS = 1
export const BOARDBAG_LISTING_MAX_PHOTOS = 12

const boardbagSizeValues = BOARDBAG_SIZE_OPTIONS.map((o) => o.value)
...
const boardbagListingImageSchema = z.object({
...
const boardbagListingBaseSchema = z.object({
  title: z.string().trim().min(3, "Add a title").max(BOARDBAG_LISTING_TITLE_MAX_LENGTH),
  ...
  size: optionalSlug(boardbagSizeValues),
  ...
  images: z
    .array(boardbagListingImageSchema)
    .min(BOARDBAG_LISTING_MIN_PHOTOS, "Add at least one photo")
    .max(BOARDBAG_LISTING_MAX_PHOTOS),
})

function withBoardbagListingRefinements<T extends z.ZodType>(schema: T) {
  return schema
...
const boardbagListingUpdateBaseSchema = boardbagListingBaseSchema.extend({
  listingId: z.string().uuid(),
  removedImageIds: z.array(z.string().uuid()).optional().default([]),
})

export const createBoardbagListingSchema = withBoardbagListingRefinements(boardbagListingBaseSchema)
...
export type CreateBoardbagListingInput = z.infer<typeof boardbagListingBaseSchema>

export const updateBoardbagListingSchema = withBoardbagListingRefinements(boardbagListingUpdateBaseSchema)

export type UpdateBoardbagListingInput = z.infer<typeof boardbagListingUpdateBaseSchema>
