...
export const SURFPACK_LISTING_TITLE_MAX_LENGTH = 60
export const SURFPACK_LISTING_MIN_PHOTOS = 1
export const SURFPACK_LISTING_MAX_PHOTOS = 12

const surfpackSizeValues = SURFPACK_SIZE_OPTIONS.map((o) => o.value)
...
const surfpackListingImageSchema = z.object({
...
const surfpackListingBaseSchema = z.object({
  title: z.string().trim().min(3, "Add a title").max(SURFPACK_LISTING_TITLE_MAX_LENGTH),
  ...
  size: optionalSlug(surfpackSizeValues),
...
export type CreateSurfpackListingInput = z.infer<typeof surfpackListingBaseSchema>

export const updateSurfpackListingSchema = withSurfpackListingRefinements(surfpackListingUpdateBaseSchema)

export type UpdateSurfpackListingInput = z.infer<typeof surfpackListingUpdateBaseSchema>
