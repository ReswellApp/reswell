/**
 * Copy-paste HTML for Klaviyo seller **Shipping Sale Received** emails.
 *
 * Reswell-label only — no seller-own / shipping-tools path.
 *
 * **Flow setup**
 * 1. Flows → Create flow → Metric → **Shipping Sale Received**
 * 2. Optional trigger filter: `label_workflow` equals `reswell`
 ...
 *
 * **Event variables** (from `track-seller-shipping-sale-received.ts`):
 ...
 * - `ship_to_formatted` (multi-line), plus `ship_to_name` / `ship_to_line1` / city / state / zip
 *
 * **Klaviyo notes**
...
/**
 * **Paste this in Klaviyo** — seller shipping sale, Reswell label.
 * Trigger: **Shipping Sale Received**.
 */
export const KLAVIYO_SELLER_SHIPPING_SALE_RECEIVED_EMAIL_HTML = `<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;background:${C.background};">
<tr>
...
<td align="center" style="padding:0 ${KLAVIYO_EMAIL_HORIZONTAL_PADDING} 24px ${KLAVIYO_EMAIL_HORIZONTAL_PADDING};">
  <p style="margin:0;font-family:${fontSans};font-size:${KLAVIYO_EMAIL_BODY_FONT_SIZE};line-height:1.5;color:${C.foreground};text-align:center;">
    {{ event|lookup:'buyer_display_name' }} bought <strong style="font-weight:600;">{{ event|lookup:'Title' }}</strong>. Reswell is preparing the shipping label. We&rsquo;ll email you when it&rsquo;s ready to print.
  </p>
</td>
</tr>
...
<td style="padding:0 ${KLAVIYO_EMAIL_HORIZONTAL_PADDING} 24px ${KLAVIYO_EMAIL_HORIZONTAL_PADDING};">
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;max-width:440px;margin:0 auto;border:1px solid ${KLAVIYO_EMAIL_BORDER};border-radius:${KLAVIYO_EMAIL_RADIUS};overflow:hidden;">
    ...
    <tr>
      <td style="padding:0 20px 16px 20px;font-family:${fontSans};font-size:15px;line-height:1.55;color:${C.foreground};white-space:pre-wrap;">{{ event|lookup:'ship_to_formatted' }}</td>
    </tr>
  </table>
</td>
</tr>
      ...
      <td style="padding:0 0 14px 0;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;">
          <tr>
            ...
            <td style="font-family:${fontSans};font-size:15px;line-height:1.5;color:${C.foreground};">Reswell buys the shipping label. Sit tight. No need to purchase one yourself.</td>
          </tr>
        </table>
      </td>
    </tr>
      ...
      <td style="padding:0 0 14px 0;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;">
          <tr>
            ...
            <td style="font-family:${fontSans};font-size:15px;line-height:1.5;color:${C.foreground};">We&rsquo;ll email you when the label is ready to download and print from your sale page.</td>
          </tr>
        </table>
      </td>
    </tr>
      ...
      <td style="padding:0 0 14px 0;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;">
          <tr>
            ...
            <td style="font-family:${fontSans};font-size:15px;line-height:1.5;color:${C.foreground};">Pack the item securely, attach the label, and drop it off with the carrier.</td>
          </tr>
        </table>
      </td>
    </tr>
