...
} from "@/lib/services/reswellPlatformReview"
import {
  reswellPlatformReviewSchema,
  soldFlowReswellReviewSchema,
...
export async function submitSoldFlowReswellReviewAction(raw: unknown) {
  const parsed = soldFlowReswellReviewSchema.safeParse(raw)
  if (!parsed.success) {
    const firstIssue = parsed.error.issues[0]?.message
    return { error: firstIssue ?? "Invalid input." as const }
  }
