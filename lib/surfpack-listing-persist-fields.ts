import {
  SURFPACKS_SECTION,
  USED_SURFPACKS_CATEGORY_ID,
  surfpackSizeSlugForDb,
} from "@/lib/surfpack-listing-config"
import { reswellPackageFieldsToDb } from "@/lib/sell-listing-fulfillment-flags"
import type { CreateSurfpackListingInput } from "@/lib/validations/surfpack-listing"

export function surfpackListingShippingFieldsFor(input: CreateSurfpackListingInput): {
...
export function buildSurfpackListingPersistFields(
  input: CreateSurfpackListingInput,
): Record<string, unknown> {
  const shipping = surfpackListingShippingFieldsFor(input)
  ...
  return {
    ...
    category_id: USED_SURFPACKS_CATEGORY_ID,
    ...
    surfpack_size: surfpackSizeSlugForDb(input.size ?? null),
    board_type: null,
