...
import { SellAccessoriesFacetFields } from "@/components/features/sell/sell-accessories-facet-fields"
...
const SELL_ACCESSORIES_FORM_SECTION_NAV_ITEMS = buildSellSectionNavItems("accessories", "Accessory details")
...
function accessoryShippingModeFromListing(listing: {
  shipping_available?: boolean | null
  shipping_price?: number | string | null
  board_shipping_cost_mode?: string | null
}): AccessoryFormState["shippingMode"] {
...
const INITIAL_STATE: AccessoryFormState = {
  title: "",
  description: "",
  price: "",
  sellerPurchasePrice: "",
...
/** Type-safe merge of an IndexedDB draft snapshot onto the accessory form state. */
function accessoryFormFromDraftSnapshot(snapshot: SellListingDraftFormSnapshot): AccessoryFormState {
  const next: AccessoryFormState = { ...INITIAL_STATE }
  for (const key of Object.keys(INITIAL_STATE) as Array<keyof AccessoryFormState>) {
    const value = snapshot[key]
    if (value === undefined) continue
    const initial = INITIAL_STATE[key]
    if (key === "locationLat" || key === "locationLng") {
      if (value === null || typeof value === "number") {
        next[key] = value as AccessoryFormState["locationLat"]
      }
      continue
    }
    if (key === "shippingMode") {
...
function scrollAccessorySellSectionIntoView(sectionId: string) {
  const el = document.getElementById(sectionId)
  if (!el) return
  el.scrollIntoView({ behavior: "smooth", block: "start" })
}

export default function SellAccessoriesFlow({ editListingId = null }: { editListingId?: string | null }) {
  ...
  const [form, setForm] = useState<AccessoryFormState>(INITIAL_STATE)
  ...
  const restoreAccessoryDraftForm = useCallback((snapshot: SellListingDraftFormSnapshot) => {
    setForm(accessoryFormFromDraftSnapshot(snapshot))
  }, [])
  ...
  const { clearRecoveredDraft, flushDraftNow } = useSellAccessoryDraftRecovery({
    ...
    onRestoreForm: restoreAccessoryDraftForm,
    setImages: noopSetDraftImages,
    retryPhotoSlot: noopRetryDraftPhotoSlot,
  })

  const hydrateAccessoryEdit = useCallback(
    async (listing: OwnedListingForEditRow) => {
      ...
      if ((listing as { section?: string }).section !== "accessories") {
        toast.error("Only accessory listings can be edited here.")
        router.replace("/sell/accessories", { scroll: false })
        return { status: "handled" as const }
      }
      ...
      const shippingMode = accessoryShippingModeFromListing(
        listing as {
          shipping_available?: boolean | null
          shipping_price?: number | string | null
          board_shipping_cost_mode?: string | null
      ...
      setForm({
        ...
        size: (listing as { accessory_size?: string | null }).accessory_size ?? "",
  ...
  const { editLoading, editLoadError, retryEditLoad } = useOwnedListingEditLoad({
    editId,
    supabase: supabaseRef.current,
    signInReturnPath: editId ? `/sell/accessories?edit=${editId}` : "/sell/accessories",
    openSignIn: signIn,
    notFoundRedirectHref: "/sell/accessories",
    router,
    onHydrate: hydrateAccessoryEdit,
  })

  const setField = useCallback(<K extends keyof AccessoryFormState>(key: K, value: AccessoryFormState[K]) => {
  ...
  const uploadSlot = useCallback(
    async (slot: PhotoSlot) => {
      ...
      try {
        ...
        console.error("accessory photo upload failed", err)
        logSellFunnelEvent({
          listingType: "accessories",
          event: "upload_failed",
  ...
  const addFiles = useCallback(
    (files: FileList | File[]) => {
      ...
      const remaining = ACCESSORY_LISTING_MAX_PHOTOS - photosRef.current.length
      if (remaining <= 0) {
        toast.error(`You can add up to ${ACCESSORY_LISTING_MAX_PHOTOS} photos.`)
        return
      }
  ...
  const sellSectionCompletion = useMemo(
    () =>
      computeAccessorySellSectionCompletion({
        title: form.title,
  ...
  useSellFunnelStepTracking({
    listingType: "accessories",
    sectionIds: SELL_ACCESSORIES_FORM_SECTION_NAV_ITEMS.map((item) => item.id),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!user || !session?.access_token) {
      ...
      signIn("/sell/accessories", { preferSignUp: true, skipSessionProbe: true })
    ...
    const failValidation = (message: string) => {
      logSellFunnelEvent({ listingType: "accessories", event: "validation_failed", message })
      toast.error(message)
    }

    if (readyPhotos.length === 0) {
      failValidation("Add at least one photo.")
      scrollAccessorySellSectionIntoView("sell-accessories-section-photos-title")
      return
    }
    if (uploadingCount > 0) {
    ...
    if (!form.condition) {
      ...
      scrollAccessorySellSectionIntoView("sell-accessories-section-details")
      return
    }
    ...
    if (!form.price.trim() || Number(form.price) <= 0) {
      ...
      scrollAccessorySellSectionIntoView("sell-accessories-section-publish")
      return
    }
    if (!form.locationCity.trim() || !form.locationState.trim()) {
      failValidation("Confirm where you're listing from.")
      scrollAccessorySellSectionIntoView("sell-accessories-section-delivery")
      return
    }
    ...
    if (form.shippingAvailable && normalizeSellShippingCostMode(form.shippingMode, submitActorIsAdmin) === "reswell") {
      ...
      if (L == null || L <= 0 || W == null || W <= 0 || H == null || H <= 0) {
        ...
        scrollAccessorySellSectionIntoView("sell-accessories-section-reswell-package")
        return
      }
    }
    if (
    ...
    const payload = {
      ...
      condition: form.condition as CreateAccessoryListingInput["condition"],
      size: form.size || null,
      brand: form.brand,
      model: form.model,
      locationCity: form.locationCity,
    ...
    try {
      ...
      if (editId) {
        ...
        if (adminImpersonatesListingOwner) {
          ...
          const res = await fetch("/api/admin/impersonate/update-listing", {
            ...
            body: JSON.stringify({
              listingId: editId,
              listing: buildAccessoryListingPersistFields(payload, { allowPrivilegedShippingModes: true }),
              removedImageIds,
              images: imageOps,
              removedVideoIds,
      ...
      await finalizePeerListingCreate({
        ...
        listingFields: buildAccessoryListingPersistFields(
          payload,
        ...
        router,
        publishStartedAt,
        successToast: "Your accessory is live!",
        setSubmitting,
        directCreate: () => createAccessoryListingAction(payload),
        onCreateSuccess: () => clearRecoveredDraft(),
      })
    } catch (err) {
      const aborted = isSellSubmitAbortError(err)
      if (!aborted) {
        console.error("accessory listing submit failed", err)
      }
      logSellFunnelEvent({
        listingType: "accessories",
        event: "publish_failed",
  ...
  if (editLoadError) {
    return (
      <SellEditLoadError
        ...
        backHref="/sell/accessories"
        backLabel="Back to sell accessory"
      />
    )
  }
  ...
  return (
    <main className={cn("flex-1 w-full pt-8 pb-16 md:pb-20 lg:pb-24", SELL_PAGE_GROUND_CLASS)}>
      <AdminBulkListingBanner section="accessories" bulkSlotId={bulkSlotId} />
      <div className="container relative mx-auto max-w-2xl min-h-[50vh] lg:max-w-6xl">
        <h1 className="sr-only">{editId ? "Edit accessory listing" : "List your accessory"}</h1>
              ...
              <BreadcrumbList className="gap-1.5 text-sm font-normal text-[#5c6b89] sm:gap-2">
                ...
                <BreadcrumbItem>
                  <BreadcrumbPage className="font-normal text-[#5c6b89]">
                    {editId ? "Edit accessory listing" : "List accessory"}
                  </BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
        ...
        <div className="flex w-full flex-col gap-8 lg:mx-auto lg:w-max lg:max-w-full lg:flex-row lg:items-start lg:gap-10 xl:gap-14">
          <div className="hidden shrink-0 lg:block lg:w-52 xl:w-56">
            <SellSectionNav
              items={SELL_ACCESSORIES_FORM_SECTION_NAV_ITEMS}
              sectionCompletion={sellSectionCompletion}
            />
          </div>
          ...
          <div className="min-w-0 w-full max-w-2xl lg:w-auto lg:max-w-3xl lg:shrink-0">
            ...
            <form onSubmit={handleSubmit} className="space-y-10 lg:space-y-12" aria-busy={submitting}>
              <SellFormSection
                sectionId="sell-accessories-section-photos-title"
                title="Photos & title"
                description="Start with clear photos, then add a short title. Buyers see these first."
                complete={sellSectionCompletion["sell-accessories-section-photos-title"] === true}
              >
                <div className="space-y-8">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-end justify-between gap-2">
                      <Label htmlFor="accessory-title">Title *</Label>
                      <span
                        className={cn(
                          "text-xs tabular-nums",
                          form.title.length > ACCESSORY_LISTING_TITLE_MAX_LENGTH
                    ...
                    <Input
                      ...
                      value={form.title}
                      maxLength={ACCESSORY_LISTING_TITLE_MAX_LENGTH}
              ...
              <SellFormSection
                sectionId="sell-accessories-section-details"
                title="Accessory details & description"
                description="Condition and details help buyers shop with confidence."
              >
                <div className="space-y-8">
                  <SellAccessoriesFacetFields
                    condition={form.condition}
                    size={form.size}
                    brand={form.brand}
                    model={form.model}
                  ...
                  <SellListingDescriptionField
                    ...
                    placeholder="Material, fit, any wear or repairs, why you're selling…"
                  />
                </div>
              </SellFormSection>

              <SellFormSection
                sectionId="sell-accessories-section-delivery"
                title="Pickup & shipping"
                description="Pin where the accessory is and choose delivery options."
              >
                <div className="space-y-8">
                  <LocationPicker
                  ...
                  <div className="space-y-4 rounded-xl border border-border bg-card p-5 shadow-sm sm:p-6">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-start gap-3">
                        <Checkbox
                          id="sell-accessories-delivery-shipping"
                        ...
                        <div className="min-w-0 space-y-0.5">
                          <Label
                            htmlFor="sell-accessories-delivery-shipping"
                      ...
                      <div className="flex items-start gap-3">
                        <Checkbox
                          id="sell-accessories-delivery-pickup"
                        ...
                        />
                        <Label
                          htmlFor="sell-accessories-delivery-pickup"
              ...
              {form.shippingAvailable && form.shippingMode === "reswell" ? (
                <SellFormSection
                  sectionId="sell-accessories-section-reswell-package"
              ...
              <SellFormSection
                sectionId="sell-accessories-section-publish"
              ...
              >
                <div className="space-y-6">
                  <SellPriceFields
                    ...
                    purchaseAccordionTitle="What you paid for the accessory"
                    ...
                    afterListingPrice={
                      <div className="rounded-xl border border-border bg-card p-5 shadow-sm sm:p-6">
                        <div className="flex gap-3">
                          ...
                          <div className="min-w-0 space-y-1">
                            <h3 className="text-sm font-semibold text-foreground">
                              Sell your accessory even faster
                            </h3>
                        ...
                        <div className="flex gap-4">
                          <Switch
                            id="sell-accessories-buyer-offers"
                          ...
                          <div className="min-w-0 space-y-1">
                            <Label
                              htmlFor="sell-accessories-buyer-offers"
