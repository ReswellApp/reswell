...
import { SellTractionFacetFields } from "@/components/features/sell/sell-traction-facet-fields"
...
import {
  TRACTION_LISTING_MAX_PHOTOS,
  TRACTION_LISTING_TITLE_MAX_LENGTH,
  type CreateTractionListingInput,
} from "@/lib/validations/traction-listing"
import { createTractionListingAction } from "@/lib/actions/tractionListingActions"
import {
  peerImagesToOwnedUpdateOps,
  updateOwnedListingViaApi,
} from "@/lib/sell-flow/update-owned-listing-client"
import { buildTractionListingPersistFields } from "@/lib/traction-listing-persist-fields"
import { computeTractionSellSectionCompletion } from "@/lib/traction-sell-section-completion"
...
const SELL_TRACTION_FORM_SECTION_NAV_ITEMS = buildSellSectionNavItems("traction", "Traction details")
...
function tractionShippingModeFromListing(listing: {
  shipping_available?: boolean | null
  shipping_price?: number | string | null
  board_shipping_cost_mode?: string | null
}): TractionFormState["shippingMode"] {
...
const INITIAL_STATE: TractionFormState = {
  title: "",
  description: "",
  price: "",
  sellerPurchasePrice: "",
...
/** Type-safe merge of an IndexedDB draft snapshot onto the traction form state. */
function tractionFormFromDraftSnapshot(snapshot: SellListingDraftFormSnapshot): TractionFormState {
  const next: TractionFormState = { ...INITIAL_STATE }
  for (const key of Object.keys(INITIAL_STATE) as Array<keyof TractionFormState>) {
    const value = snapshot[key]
    if (value === undefined) continue
    const initial = INITIAL_STATE[key]
    if (key === "locationLat" || key === "locationLng") {
      if (value === null || typeof value === "number") {
        next[key] = value as TractionFormState["locationLat"]
      }
      continue
    }
    if (key === "shippingMode") {
...
function scrollTractionSellSectionIntoView(sectionId: string) {
  const el = document.getElementById(sectionId)
  if (!el) return
  el.scrollIntoView({ behavior: "smooth", block: "start" })
}

export default function SellTractionFlow({ editListingId = null }: { editListingId?: string | null }) {
  ...
  const [form, setForm] = useState<TractionFormState>(INITIAL_STATE)
  ...
  const restoreTractionDraftForm = useCallback((snapshot: SellListingDraftFormSnapshot) => {
    setForm(tractionFormFromDraftSnapshot(snapshot))
  }, [])

  const { clearRecoveredDraft, flushDraftNow } = useSellAccessoryDraftRecovery({
    listingType: "traction",
    editId,
    startFresh,
    formSnapshot: form,
    images: NO_DRAFT_PHOTO_SLOTS,
    onRestoreForm: restoreTractionDraftForm,
    setImages: noopSetDraftImages,
    retryPhotoSlot: noopRetryDraftPhotoSlot,
  })

  const hydrateTractionEdit = useCallback(
    async (listing: OwnedListingForEditRow) => {
      ...
      if ((listing as { section?: string }).section !== "traction") {
        toast.error("Only traction listings can be edited here.")
        router.replace("/sell/traction", { scroll: false })
        return { status: "handled" as const }
      }
      ...
      const shippingMode = tractionShippingModeFromListing(
        listing as {
          shipping_available?: boolean | null
          shipping_price?: number | string | null
          board_shipping_cost_mode?: string | null
      ...
      setForm({
        ...
        size: (listing as { traction_size?: string | null }).traction_size ?? "",
  ...
  const { editLoading, editLoadError, retryEditLoad } = useOwnedListingEditLoad({
    editId,
    supabase: supabaseRef.current,
    signInReturnPath: editId ? `/sell/traction?edit=${editId}` : "/sell/traction",
    openSignIn: signIn,
    notFoundRedirectHref: "/sell/traction",
    router,
    onHydrate: hydrateTractionEdit,
  })

  const setField = useCallback(<K extends keyof TractionFormState>(key: K, value: TractionFormState[K]) => {
  ...
  const uploadSlot = useCallback(
    async (slot: PhotoSlot) => {
      ...
      try {
        ...
        console.error("traction photo upload failed", err)
        logSellFunnelEvent({
          listingType: "traction",
          event: "upload_failed",
  ...
  const addFiles = useCallback(
    (files: FileList | File[]) => {
      ...
      const remaining = TRACTION_LISTING_MAX_PHOTOS - photosRef.current.length
      if (remaining <= 0) {
        toast.error(`You can add up to ${TRACTION_LISTING_MAX_PHOTOS} photos.`)
        return
      }
  ...
  const sellSectionCompletion = useMemo(
    () =>
      computeTractionSellSectionCompletion({
        title: form.title,
  ...
  useSellFunnelStepTracking({
    listingType: "traction",
    sectionIds: SELL_TRACTION_FORM_SECTION_NAV_ITEMS.map((item) => item.id),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!user || !session?.access_token) {
      ...
      signIn("/sell/traction", { preferSignUp: true, skipSessionProbe: true })
    ...
    const failValidation = (message: string) => {
      logSellFunnelEvent({ listingType: "traction", event: "validation_failed", message })
      toast.error(message)
    }

    if (readyPhotos.length === 0) {
      failValidation("Add at least one photo.")
      scrollTractionSellSectionIntoView("sell-traction-section-photos-title")
      return
    }
    if (uploadingCount > 0) {
    ...
    if (!form.condition) {
      ...
      scrollTractionSellSectionIntoView("sell-traction-section-details")
      return
    }
    ...
    if (!form.price.trim() || Number(form.price) <= 0) {
      ...
      scrollTractionSellSectionIntoView("sell-traction-section-publish")
      return
    }
    ...
    if (form.shippingAvailable && normalizeSellShippingCostMode(form.shippingMode, submitActorIsAdmin) === "reswell") {
      ...
      if (L == null || L <= 0 || W == null || W <= 0 || H == null || H <= 0) {
        ...
        scrollTractionSellSectionIntoView("sell-traction-section-reswell-package")
        return
      }
    }
    if (
    ...
    const payload = {
      ...
      condition: form.condition as CreateTractionListingInput["condition"],
      size: form.size || null,
      brand: form.brand,
      model: form.model,
      locationCity: form.locationCity,
    ...
    try {
      ...
      if (editId) {
        ...
        if (adminImpersonatesListingOwner) {
          ...
          const res = await fetch("/api/admin/impersonate/update-listing", {
            ...
            body: JSON.stringify({
              listingId: editId,
              listing: buildTractionListingPersistFields(payload, { allowPrivilegedShippingModes: true }),
              removedImageIds,
              images: imageOps,
              removedVideoIds,
        ...
        const updated = await updateOwnedListingViaApi({
          listingId: editId,
          listing: buildTractionListingPersistFields(payload),
          removedImageIds,
      ...
      await finalizePeerListingCreate({
        ...
        listingFields: buildTractionListingPersistFields(
          payload,
        ...
        router,
        publishStartedAt,
        successToast: "Your traction is live!",
        setSubmitting,
        directCreate: () => createTractionListingAction(payload),
        onCreateSuccess: () => clearRecoveredDraft(),
      })
    } catch (err) {
      const aborted = isSellSubmitAbortError(err)
      if (!aborted) {
        console.error("traction listing submit failed", err)
      }
      logSellFunnelEvent({
        listingType: "traction",
        event: "publish_failed",
  ...
  if (editLoadError) {
    return (
      <SellEditLoadError
        ...
        backHref="/sell/traction"
        backLabel="Back to sell traction"
      />
    )
  }
  ...
  return (
    <main className={cn("flex-1 w-full pt-8 pb-16 md:pb-20 lg:pb-24", SELL_PAGE_GROUND_CLASS)}>
      <AdminBulkListingBanner section="traction" bulkSlotId={bulkSlotId} />
      <div className="container relative mx-auto max-w-2xl min-h-[50vh] lg:max-w-6xl">
        <h1 className="sr-only">{editId ? "Edit traction listing" : "List your traction"}</h1>
              ...
              <BreadcrumbList className="gap-1.5 text-sm font-normal text-[#5c6b89] sm:gap-2">
                ...
                <BreadcrumbItem>
                  <BreadcrumbPage className="font-normal text-[#5c6b89]">
                    {editId ? "Edit traction listing" : "List traction"}
                  </BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
        ...
        <div className="flex w-full flex-col gap-8 lg:mx-auto lg:w-max lg:max-w-full lg:flex-row lg:items-start lg:gap-10 xl:gap-14">
          <div className="hidden shrink-0 lg:block lg:w-52 xl:w-56">
            <SellSectionNav
              items={SELL_TRACTION_FORM_SECTION_NAV_ITEMS}
              sectionCompletion={sellSectionCompletion}
            />
          </div>
          ...
          <div className="min-w-0 w-full max-w-2xl lg:w-auto lg:max-w-3xl lg:shrink-0">
            ...
            <form onSubmit={handleSubmit} className="space-y-10 lg:space-y-12" aria-busy={submitting}>
              <SellFormSection
                sectionId="sell-traction-section-photos-title"
                title="Photos & title"
                description="Start with clear photos, then add a short title. Buyers see these first."
                complete={sellSectionCompletion["sell-traction-section-photos-title"] === true}
              >
                <div className="space-y-8">
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-4 gap-2 sm:grid-cols-5">
                      ...
                      {photos.length < TRACTION_LISTING_MAX_PHOTOS ? (
                  ...
                  <div className="space-y-2">
                    <div className="flex items-end justify-between gap-2">
                      <Label htmlFor="traction-title">Title *</Label>
                      <span
                        className={cn(
                          "text-xs tabular-nums",
                          form.title.length > TRACTION_LISTING_TITLE_MAX_LENGTH
                    ...
                    <Input
                      ...
                      value={form.title}
                      maxLength={TRACTION_LISTING_TITLE_MAX_LENGTH}
              ...
              <SellFormSection
                sectionId="sell-traction-section-details"
                title="Traction details & description"
                description="Condition and details help buyers shop with confidence."
              >
                <div className="space-y-8">
                  <SellTractionFacetFields
                    condition={form.condition}
                    size={form.size}
                    brand={form.brand}
                    model={form.model}
              ...
              <SellFormSection
                sectionId="sell-traction-section-delivery"
                title="Pickup & shipping"
                description="Pin where the traction is and choose delivery options."
              >
                <div className="space-y-8">
                  <LocationPicker
              ...
              <SellFormSection
                sectionId="sell-traction-section-publish"
              ...
              >
                <div className="space-y-6">
                  <SellPriceFields
                    ...
                    purchaseAccordionTitle="What you paid for the traction"
                    ...
                    afterListingPrice={
                      <div className="rounded-xl border border-border bg-card p-5 shadow-sm sm:p-6">
                        <div className="flex gap-3">
                          ...
                          <div className="min-w-0 space-y-1">
                            <h3 className="text-sm font-semibold text-foreground">
                              Sell your traction even faster
                            </h3>
                        ...
                        <div className="flex gap-4">
                          ...
                          <div className="min-w-0 space-y-1">
                            <Label
                              htmlFor="sell-traction-buyer-offers"
