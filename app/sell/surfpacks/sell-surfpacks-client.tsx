...
import { SellSurfpacksFacetFields } from "@/components/features/sell/sell-surfpacks-facet-fields"
...
const SELL_SURFPACKS_FORM_SECTION_NAV_ITEMS = buildSellSectionNavItems("surfpacks", "Surfpack details")
...
function surfpackShippingModeFromListing(listing: {
  shipping_available?: boolean | null
  shipping_price?: number | string | null
  board_shipping_cost_mode?: string | null
}): SurfpackFormState["shippingMode"] {
...
const INITIAL_STATE: SurfpackFormState = {
  title: "",
  description: "",
  price: "",
  sellerPurchasePrice: "",
...
/** Type-safe merge of an IndexedDB draft snapshot onto the surfpack form state. */
function surfpackFormFromDraftSnapshot(snapshot: SellListingDraftFormSnapshot): SurfpackFormState {
  const next: SurfpackFormState = { ...INITIAL_STATE }
  for (const key of Object.keys(INITIAL_STATE) as Array<keyof SurfpackFormState>) {
    const value = snapshot[key]
    if (value === undefined) continue
    const initial = INITIAL_STATE[key]
    if (key === "locationLat" || key === "locationLng") {
      if (value === null || typeof value === "number") {
        next[key] = value as SurfpackFormState["locationLat"]
      }
      continue
    }
    if (key === "shippingMode") {
...
function scrollSurfpackSellSectionIntoView(sectionId: string) {
  const el = document.getElementById(sectionId)
  if (!el) return
  el.scrollIntoView({ behavior: "smooth", block: "start" })
}

export default function SellSurfpacksFlow({ editListingId = null }: { editListingId?: string | null }) {
  ...
  const [form, setForm] = useState<SurfpackFormState>(INITIAL_STATE)
  ...
  const restoreSurfpackDraftForm = useCallback((snapshot: SellListingDraftFormSnapshot) => {
    setForm(surfpackFormFromDraftSnapshot(snapshot))
  }, [])
  ...
  const { clearRecoveredDraft, flushDraftNow } = useSellAccessoryDraftRecovery({
    ...
    onRestoreForm: restoreSurfpackDraftForm,
    setImages: noopSetDraftImages,
    retryPhotoSlot: noopRetryDraftPhotoSlot,
  })

  const hydrateSurfpackEdit = useCallback(
    (listing: OwnedListingForEditRow) => {
      ...
      if ((listing as { section?: string }).section !== "surfpacks") {
        toast.error("Only surfpack listings can be edited here.")
        router.replace("/sell/surfpacks", { scroll: false })
        return { status: "handled" as const }
      }
      ...
      const shippingMode = surfpackShippingModeFromListing(
        listing as {
          shipping_available?: boolean | null
          shipping_price?: number | string | null
          board_shipping_cost_mode?: string | null
  ...
  const { editLoading, editLoadError, retryEditLoad } = useOwnedListingEditLoad({
    editId,
    supabase: supabaseRef.current,
    signInReturnPath: editId ? `/sell/surfpacks?edit=${editId}` : "/sell/surfpacks",
    openSignIn: signIn,
    notFoundRedirectHref: "/sell/surfpacks",
    router,
    onHydrate: hydrateSurfpackEdit,
  })

  const setField = useCallback(<K extends keyof SurfpackFormState>(key: K, value: SurfpackFormState[K]) => {
  ...
  const addFiles = useCallback(
    (files: FileList | File[]) => {
      ...
      if (remaining <= 0) {
        toast.error(`You can add up to ${SURFPACK_LISTING_MAX_PHOTOS} photos.`)
        return
      }
  ...
  const sellSectionCompletion = useMemo(
    () =>
      computeSurfpackSellSectionCompletion({
        title: form.title,
  ...
  useSellFunnelStepTracking({
    listingType: "surfpacks",
    sectionIds: SELL_SURFPACKS_FORM_SECTION_NAV_ITEMS.map((item) => item.id),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const failValidation = (message: string) => {
      logSellFunnelEvent({ listingType: "surfpacks", event: "validation_failed", message })
      toast.error(message)
    }

    if (readyPhotos.length === 0) {
      failValidation("Add at least one photo.")
      scrollSurfpackSellSectionIntoView("sell-surfpacks-section-photos-title")
      return
    }
    if (uploadingCount > 0) {
    ...
    if (!form.condition) {
      ...
      scrollSurfpackSellSectionIntoView("sell-surfpacks-section-details")
      return
    }
    ...
    if (!form.price.trim() || Number(form.price) <= 0) {
      ...
      scrollSurfpackSellSectionIntoView("sell-surfpacks-section-publish")
      return
    }
    if (!form.locationCity.trim() || !form.locationState.trim()) {
      failValidation("Confirm where you're listing from.")
      scrollSurfpackSellSectionIntoView("sell-surfpacks-section-delivery")
      return
    }
    ...
    if (form.shippingAvailable && normalizeSellShippingCostMode(form.shippingMode, submitActorIsAdmin) === "reswell") {
      ...
      if (L == null || L <= 0 || W == null || W <= 0 || H == null || H <= 0) {
        ...
        scrollSurfpackSellSectionIntoView("sell-surfpacks-section-reswell-package")
        return
      }
    }
    if (
    ...
    const payload = {
      ...
      condition: form.condition as CreateSurfpackListingInput["condition"],
      size: form.size || null,
      brand: form.brand,
      model: form.model,
      locationCity: form.locationCity,
    ...
    try {
      ...
      if (editId) {
        ...
        if (adminImpersonatesListingOwner) {
          ...
          const res = await fetch("/api/admin/impersonate/update-listing", {
            ...
            body: JSON.stringify({
              listingId: editId,
              listing: buildSurfpackListingPersistFields(payload, { allowPrivilegedShippingModes: true }),
              removedImageIds,
              images: imageOps,
              removedVideoIds,
      ...
      await finalizePeerListingCreate({
        listingImpersonation,
        listingFields: buildSurfpackListingPersistFields(
          payload,
        ...
        router,
        publishStartedAt,
        successToast: "Your surfpack is live!",
        setSubmitting,
        directCreate: () => createSurfpackListingAction(payload),
        onCreateSuccess: () => clearRecoveredDraft(),
      })
    } catch (err) {
      const aborted = isSellSubmitAbortError(err)
      if (!aborted) {
        console.error("surfpack listing submit failed", err)
      }
      logSellFunnelEvent({
        listingType: "surfpacks",
        event: "publish_failed",
  ...
  if (editLoadError) {
    return (
      <SellEditLoadError
        ...
        backHref="/sell/surfpacks"
        backLabel="Back to sell surfpack"
      />
    )
  }
      ...
      <div className="container relative mx-auto max-w-2xl min-h-[50vh] lg:max-w-6xl">
              ...
              <BreadcrumbList className="gap-1.5 text-sm font-normal text-[#5c6b89] sm:gap-2">
                ...
                <BreadcrumbItem>
                  <BreadcrumbPage className="font-normal text-[#5c6b89]">
                    {editId ? "Edit surfpack listing" : "List surfpack"}
                  </BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
        ...
        <div className="flex w-full flex-col gap-8 lg:mx-auto lg:w-max lg:max-w-full lg:flex-row lg:items-start lg:gap-10 xl:gap-14">
          <div className="hidden shrink-0 lg:block lg:w-52 xl:w-56">
            <SellSectionNav
              items={SELL_SURFPACKS_FORM_SECTION_NAV_ITEMS}
              sectionCompletion={sellSectionCompletion}
            />
          </div>
          ...
          <div className="min-w-0 w-full max-w-2xl lg:w-auto lg:max-w-3xl lg:shrink-0">
            ...
            <form onSubmit={handleSubmit} className="space-y-10 lg:space-y-12" aria-busy={submitting}>
              <SellFormSection
                sectionId="sell-surfpacks-section-photos-title"
                title="Photos & title"
                description="Start with clear photos, then add a short title. Buyers see these first."
                complete={sellSectionCompletion["sell-surfpacks-section-photos-title"] === true}
              >
                <div className="space-y-8">
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-4 gap-2 sm:grid-cols-5">
                      ...
                      {photos.length < SURFPACK_LISTING_MAX_PHOTOS ? (
                  ...
                  <div className="space-y-2">
                    <div className="flex items-end justify-between gap-2">
                      <Label htmlFor="surfpack-title">Title *</Label>
                      <span
                        className={cn(
              ...
              <SellFormSection
                sectionId="sell-surfpacks-section-details"
                title="Surfpack details & description"
                description="Condition and details help buyers shop with confidence."
              >
                <div className="space-y-8">
                  <SellSurfpacksFacetFields
                    condition={form.condition}
                    size={form.size}
                    brand={form.brand}
                    model={form.model}
              ...
              <SellFormSection
                sectionId="sell-surfpacks-section-delivery"
                title="Pickup & shipping"
                description="Pin where the surfpack is and choose delivery options."
              >
                <div className="space-y-8">
                  <LocationPicker
                  ...
                  <div className="space-y-4 rounded-xl border border-border bg-card p-5 shadow-sm sm:p-6">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-start gap-3">
                        <Checkbox
                          id="sell-surfpacks-delivery-shipping"
                        ...
                        <div className="min-w-0 space-y-0.5">
                          <Label
                            htmlFor="sell-surfpacks-delivery-shipping"
                      ...
                      <div className="flex items-start gap-3">
                        <Checkbox
                          id="sell-surfpacks-delivery-pickup"
                        ...
                        />
                        <Label
                          htmlFor="sell-surfpacks-delivery-pickup"
                    ...
                    <div className="space-y-4 rounded-xl border border-border bg-card p-5 shadow-sm sm:p-6">
                      ...
                      <SellShippingCostModeRadios
                        ...
                        <div className="space-y-2 rounded-lg border border-border bg-background p-4 sm:p-5">
                          <Label htmlFor="surfpack-shipping-price" className="text-sm font-semibold text-foreground">
                            Shipping rate{" "}
                            <span className="text-destructive" aria-hidden>
                              *
                            </span>
              ...
              {form.shippingAvailable && form.shippingMode === "reswell" ? (
                <SellFormSection
                  sectionId="sell-surfpacks-section-reswell-package"
              ...
              <SellFormSection
                sectionId="sell-surfpacks-section-publish"
                    ...
                    afterListingPrice={
                      <div className="rounded-xl border border-border bg-card p-5 shadow-sm sm:p-6">
                        ...
                        <div className="flex gap-4">
                          <Switch
                            id="sell-surfpacks-buyer-offers"
                          ...
                          <div className="min-w-0 space-y-1">
                            <Label
                              htmlFor="sell-surfpacks-buyer-offers"
