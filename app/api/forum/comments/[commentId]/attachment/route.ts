...
import { z } from "zod"
import { authorizeForumCommentAttachmentDownload } from "@/lib/services/forumCommentAttachmentAccess"
...
export async function HEAD(
  ...
  const auth = await authorizeForumCommentAttachmentDownload(parsed.data)
  if (!auth.ok) {
...
export async function GET(
  ...
  if (dlErr || !blob) {
    console.error("[forum comment attachment GET] download:", dlErr)
