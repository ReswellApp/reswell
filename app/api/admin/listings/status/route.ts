...
import { setAdminListingStatus } from "@/lib/services/adminListingStatus"
import { adminListingStatusBulkBodySchema } from "@/lib/validations/admin-listing-status"
...
export async function PATCH(request: NextRequest) {
  ...
  const parsed = adminListingStatusBulkBodySchema.safeParse(body)
  if (!parsed.success) {
  ...
  return NextResponse.json({ success: true, updated_ids: result.updatedIds })
}
