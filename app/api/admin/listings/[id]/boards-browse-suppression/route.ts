...
import { listingBoardsBrowseSuppressionBodySchema } from "@/lib/validations/listing-boards-browse-suppression"
...
export async function PATCH(
  ...
  const parsed = listingBoardsBrowseSuppressionBodySchema.safeParse(body)
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 })
  }

  const result = await setListingBoardsBrowseSuppression({
    listingId: listingId.trim(),
    suppressed: parsed.data.suppressed_on_boards_browse,
  })
