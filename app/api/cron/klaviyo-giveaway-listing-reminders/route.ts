...
/**
 * Hourly job: raffle entries still pending (no published surfboard) after 2+ hours
 * → Klaviyo **Giveaway Listing Reminder**.
...
export async function GET(request: Request) {
  ...
  try {
    const summary = await processGiveawayListingReminders(supabase, { referenceTime })
