...
import { bootstrapPlatformErrorDigest } from "@/lib/klaviyo/bootstrap-platform-error-digest"
import { NextResponse } from "next/server"

/**
 * One-time (or rare) call so **Platform Error Digest** appears under
...
export async function POST(request: Request) {
  ...
  try {
    const { result } = await bootstrapPlatformErrorDigest()
