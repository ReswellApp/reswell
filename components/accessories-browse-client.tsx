...
import { AccessoriesBrowseFilterToolbar } from "@/components/accessories-browse-filter-toolbar"
import { AccessoriesBrowseFacetControls } from "@/components/accessories-browse-facet-controls"
import { useAccessoriesFilterState } from "@/components/accessories-browse-filter-state"
import { ACCESSORY_FACET_PARAM_KEYS, accessoryFacetOptionLabel } from "@/lib/accessories-browse-facets"
import { cn } from "@/lib/utils"

type ActiveChip = { id: string; label: string; onRemove: () => void }

export function AccessoriesBrowseClient({ children }: { children: ReactNode }) {
  const [isPending, startTransition] = useTransition()
  const [mobileOpen, setMobileOpen] = useState(false)
  const [desktopFiltersOpen, setDesktopFiltersOpen] = useState(false)
  const state = useAccessoriesFilterState(startTransition)
  ...
  const chips = useMemo<ActiveChip[]>(() => {
    ...
    const pushMulti = (paramKey: string, values: string[]) => {
      for (const v of values) {
        out.push({
          id: `${paramKey}:${v}`,
          label: accessoryFacetOptionLabel(paramKey, v),
          onRemove: () => state.toggleMulti(paramKey, v),
        })
      }
    }
    pushMulti(ACCESSORY_FACET_PARAM_KEYS.size, state.selections.sizes)
    pushMulti(ACCESSORY_FACET_PARAM_KEYS.condition, state.selections.conditions)
  ...
  return (
    <>
      <div className="w-full min-w-0 border-b pb-4">
        <AccessoriesBrowseFilterToolbar
      ...
      <div className="mt-4 flex w-full min-w-0 gap-6">
        <aside className={cn("hidden w-[260px] shrink-0", desktopFiltersOpen && "md:block")}>
          <div className="sticky top-4">
            ...
            <AccessoriesBrowseFacetControls state={state} />
          </div>
        </aside>
