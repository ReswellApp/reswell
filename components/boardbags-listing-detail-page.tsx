...
import { BOARDBAGS_SECTION, boardbagSizeLabel } from "@/lib/boardbag-listing-config"
...
const SELLER_BOARDBAGS_PDP_LIMIT = 12

export async function BoardbagsListingDetailPage(props: ListingDetailPageSharedProps) {
  return renderListingDetailWithGuestFallback(props, renderBoardbagsListingDetailPage)
}

async function renderBoardbagsListingDetailPage({
  listingParam,
  prefetchedListing,
  viewerUser,
  ...
  const sellerId = boardbag.user_id as string
  ...
  const isOwnListing = user?.id === boardbag.user_id

  const pickupOffered = boardbag.local_pickup !== false
  const shippingOffered = !!boardbag.shipping_available

  const purchaseVisibility = {
    status: String(boardbag.status ?? ""),
    title: boardbag.title as string | null | undefined,
    hidden_from_site: boardbag.hidden_from_site as boolean | null | undefined,
    archived_at: boardbag.archived_at as string | null | undefined,
  }
  ...
  const modelForSpecs = (boardbag.model as string | null)?.trim() || null
  const modelPagePath = await resolveListingModelPageHref(supabase, {
    brand: indexBrand,
    brandModelId: (boardbag.brand_model_id as string | null),
    modelName: modelForSpecs,
  })

  const sizeLabel = boardbagSizeLabel(boardbag.boardbag_size as string | null)
  ...
  const buyerOffersOn = (boardbag.buyer_offers_enabled as boolean | null) !== false
  const offerPct = effectiveMinimumOfferPct(boardbag as { minimum_offer_pct?: number | null })
  ...
  const shippingFlatRate = Math.max(0, Number.parseFloat(String(boardbag.shipping_price ?? 0)) || 0)
  ...
  const listingLocationLine =
    boardbag.city && boardbag.state
      ? `${boardbag.city}, ${boardbag.state}`
      : (boardbag.profiles as { location?: string | null } | null)?.location?.trim() || null

  const boardShippingCostMode =
    (boardbag.board_shipping_cost_mode as "reswell" | "flat" | "free" | null) ?? null

  const fulfillmentLabels = boardFulfillmentDetailLabels(
    boardbag.local_pickup,
    boardbag.shipping_available,
    boardbag.shipping_price,
    boardShippingCostMode,
  )

  const conditionWords = formatCondition(boardbag.condition as string | null)
  ...
  const aboutSellerSection = (
    <ListingAboutSellerSection
      profiles={boardbag.profiles as AboutSellerProfilesProp}
      listingImageFallbacks={[{ listing_images: boardbag.listing_images }]}
      sellerProfileHref={sellerProfileHref(boardbag.profiles)}
      messageHrefAuthenticated={`/messages/new?user=${boardbag.user_id}&listing=${boardbag.id}`}
      messageHrefLoginRedirect={`/auth/login?redirect=${encodeURIComponent(boardbagHref)}`}
      isLoggedIn={!!user}
      isOwnListing={isOwnListing}
      isSold={isSold}
      avgRating={sellerAvgRating}
      reviewCount={sellerReviewCount}
      itemsSold={Number((boardbag.profiles as { sales_count?: number } | null)?.sales_count ?? 0)}
      ...
      <div className="container mx-auto w-full min-w-0 max-w-full px-4 sm:px-6 lg:px-8 lg:!max-w-[min(100%,1320px)] xl:!max-w-[min(100%,1480px)] 2xl:!max-w-[min(100%,1680px)]">
        <div className="mb-3 min-w-0 max-w-full pt-0.5 max-lg:mb-4 lg:mb-8">
          <Breadcrumb>
            <BreadcrumbList className="gap-1 text-[13px] font-normal tracking-wide text-muted-foreground sm:gap-1.5 sm:text-[14px]">
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild className="transition-colors hover:text-foreground">
                  <Link href="/boardbags">Boardbags</Link>
                </BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator className="text-muted-foreground/70 [&>svg]:stroke-[1.25]" />
              <BreadcrumbItem>
        ...
        <div className="mx-auto grid w-full min-w-0 max-w-full gap-x-8 gap-y-2 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] lg:grid-rows-[auto_auto_auto] lg:[grid-template-areas:'gallery_details'_'about_details'_'similar_similar'] lg:items-start lg:gap-x-12 lg:gap-y-0 xl:gap-x-16">
          ...
          <div className="min-w-0 max-w-full max-lg:order-2 lg:hidden">
            <ListingMobileBuySummary
              listingId={boardbag.id}
              isLoggedIn={!!user}
              condition={boardbag.condition as string | null}
              ...
              showScarcity={canPeerPurchase && boardbag.status === "active"}
              views={listingViews}
          ...
          <div className="min-w-0 space-y-5 max-lg:order-4 lg:[grid-area:details] lg:order-none lg:pt-1">
              ...
              {isSold ? (
                ...
                <>
                  <div className="mt-4">
                    ...
                    {listingPurchasable ? (
                      <ListingKlarnaAsLowAs listingId={boardbag.id} isLoggedIn={!!user} className="mt-2" />
                    ) : null}
                  </div>
            ...
            {!isOwnListing ? (
              <div className="border-b border-neutral-200/90 pb-4 dark:border-neutral-700/70">
                <Link
                  href={`/contact?topic=listing-report&listing=${encodeURIComponent(boardbag.id)}`}
                ...
                <AccordionContent className="pb-6 pt-0">
                  <div className="text-[16px] leading-[1.65] text-foreground">
                    <TranslateableDescription
                      text={(boardbag.description as string) || ""}
              ...
              {!isOwnListing && !isSold ? (
                <AccordionItem value="contact" className="border-border/55">
                  ...
                  <AccordionContent className="pb-6 pt-0">
                    <ContactSellerForm
                      listingId={boardbag.id}
                      listingSlug={boardbag.slug}
                      sellerId={boardbag.user_id}
        ...
        {sellerBoardbags && sellerBoardbags.length > 0 && (
          <section className="mt-16 min-w-0 w-full border-t border-neutral-200/90 pt-12 dark:border-neutral-700/70">
            <h2 className="mb-8 text-2xl font-bold text-foreground">More boardbags from this seller</h2>
            <HomeListingScrollRow uniformCardHeights>
              {sellerBoardbags.map((item) => (
                <HomePeerListingScrollTile
                  key={item.id}
                  listing={item}
                  userId={user?.id ?? null}
                  isFavorited={sellerBoardbagFavoritedIds.includes(item.id)}
                />
              ))}
            </HomeListingScrollRow>
          </section>
