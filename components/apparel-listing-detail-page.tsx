...
export async function ApparelListingDetailPage({
  ...
  const { listing: apparelRaw } = await findListingByParam(supabase, listingParam, {
  ...
  let sellerApparelFavoritedIds: string[] = []
  if (user && sellerApparelIds.length > 0) {
    const { data: favs } = await supabase
      .from("favorites")
      .select("listing_id")
      .eq("user_id", user.id)
      .in("listing_id", sellerApparelIds)
    sellerApparelFavoritedIds = (favs ?? []).map((f) => f.listing_id)
  }
  ...
  if (user && !isOwnListing && apparel.status === "active") {
    const accepted = await fetchAcceptedOfferForBuyerListing(supabase, user.id, apparel.id)
    if (accepted && accepted.seller_id === apparel.user_id) {
