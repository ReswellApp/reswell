...
export async function ApparelListingDetailPage({
  ...
  const sellerId = apparel.user_id as string
  const isSold = apparel.status === "sold"
  const apparelHref = listingDetailHref({ id: apparel.id as string, slug: apparel.slug as string | null })
  ...
  const canPeerPurchase = peerListingCanPurchase(listingInventoryFieldsFromRow(apparel as Record<string, unknown>), { isOwnListing })
  ...
  if (acceptedOffer && acceptedOffer.seller_id === apparel.user_id) {
  ...
  const fulfillmentLabels = boardFulfillmentDetailLabels(
    ...
    apparel.shipping_available,
    apparel.shipping_price,
    boardShippingCostMode,
  )
  const specSubline = fulfillmentLabels.length > 0 ? fulfillmentLabels.join(" · ") : null

  const conditionWords = formatCondition(apparel.condition as string | null)
  ...
  if (apparel.created_at != null) {
    const d = new Date(apparel.created_at as string | number | Date)
  ...
  const aboutSellerSection = (
    <ListingAboutSellerSection
      profiles={apparel.profiles as AboutSellerProfilesProp}
      listingImageFallbacks={[{ listing_images: apparel.listing_images }]}
      sellerProfileHref={sellerProfileHref(apparel.profiles)}
      messageHrefAuthenticated={`/messages/new?user=${apparel.user_id}&listing=${apparel.id}`}
      messageHrefLoginRedirect={`/auth/login?redirect=${encodeURIComponent(apparelHref)}`}
      isLoggedIn={!!user}
      isOwnListing={isOwnListing}
      isSold={isSold}
      avgRating={sellerAvgRating}
      reviewCount={sellerReviewCount}
      itemsSold={Number((apparel.profiles as { sales_count?: number } | null)?.sales_count ?? 0)}
      ...
      <div className="container mx-auto w-full min-w-0 max-w-full px-4 sm:px-6 lg:px-8 lg:!max-w-[min(100%,1320px)] xl:!max-w-[min(100%,1480px)] 2xl:!max-w-[min(100%,1680px)]">
        ...
        <div className="mx-auto grid w-full min-w-0 max-w-full gap-x-8 gap-y-5 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:grid-rows-[auto_auto_auto] lg:[grid-template-areas:'gallery_details'_'about_details'_'similar_similar'] lg:items-start lg:gap-x-12 lg:gap-y-0 xl:gap-x-16">
          ...
          <div className="min-w-0 max-w-full max-lg:order-2 lg:hidden">
            ...
            {canPeerPurchase ? (
              <ListingStockAvailabilityMessage listing={listingInventoryFieldsFromRow(apparel as Record<string, unknown>)} className="mt-3" />
            ) : null}
            {canPeerPurchase ? (
              <div className="mt-5">
                <ListingDetailPeerPurchaseActions
                  ...
                  section="apparel"
                  isLoggedIn={!!user}
                  hasVariants={Boolean((apparel as { has_variants?: boolean }).has_variants)}
          ...
          <div className="min-w-0 space-y-5 max-lg:order-4 lg:[grid-area:details] lg:order-none lg:pt-1">
            <div className="hidden lg:block">
              ...
              {canPeerPurchase ? (
                <ListingStockAvailabilityMessage listing={listingInventoryFieldsFromRow(apparel as Record<string, unknown>)} className="mt-4" />
              ) : null}
              {!isSold && !isOwnListing ? (
            ...
            {!isOwnListing ? (
              <div className="border-b border-neutral-200/90 pb-4 dark:border-neutral-700/70">
                <Link
                  href={`/contact?topic=listing-report&listing=${encodeURIComponent(apparel.id)}`}
                ...
                <AccordionContent className="pb-6 pt-0">
                  <div className="text-[16px] leading-[1.65] text-foreground">
                    <TranslateableDescription
                      text={(apparel.description as string) || ""}
              ...
              {!isOwnListing && !isSold ? (
                <AccordionItem value="contact" className="border-border/55">
                  ...
                  <AccordionContent className="pb-6 pt-0">
                    <ContactSellerForm
                      listingId={apparel.id}
                      listingSlug={apparel.slug}
                      sellerId={apparel.user_id}
        ...
        {sellerApparel && sellerApparel.length > 0 && (
          <section className="mt-16 min-w-0 w-full border-t border-neutral-200/90 pt-12 dark:border-neutral-700/70">
            <h2 className="mb-8 text-2xl font-bold text-foreground">More apparel from this seller</h2>
            <HomeListingScrollRow uniformCardHeights>
              {sellerApparel.map((item) => (
                <HomePeerListingScrollTile
                  key={item.id}
                  listing={item}
                  userId={user?.id ?? null}
                  isFavorited={sellerApparelFavoritedIds.includes(item.id)}
                />
              ))}
            </HomeListingScrollRow>
          </section>
