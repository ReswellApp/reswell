...
export function LeashesBrowseClient({ children }: { children: ReactNode }) {
  ...
  const state = useLeashesFilterState(startTransition)
  ...
  const chips = useMemo<ActiveChip[]>(() => {
    ...
    const pushMulti = (paramKey: string, values: string[]) => {
      for (const v of values) {
        out.push({
          id: `${paramKey}:${v}`,
          label: leashFacetOptionLabel(paramKey, v),
          onRemove: () => state.toggleMulti(paramKey, v),
        })
      }
    }
    pushMulti(LEASH_FACET_PARAM_KEYS.size, state.selections.sizes)
    pushMulti(LEASH_FACET_PARAM_KEYS.condition, state.selections.conditions)
      ...
      <div className="mt-4 flex w-full min-w-0 gap-6">
        <aside className={cn("hidden w-[260px] shrink-0", desktopFiltersOpen && "md:block")}>
          <div className="sticky top-4">
            ...
            <LeashesBrowseFacetControls state={state} />
          </div>
        </aside>
