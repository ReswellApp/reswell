...
import { FinsBrowseFilterToolbar } from "@/components/fins-browse-filter-toolbar"
import { FinsBrowseFacetControls } from "@/components/fins-browse-facet-controls"
...
export function FinsBrowseClient({
  ...
  const state = useFinsFilterState(startTransition)
  ...
  const chips = useMemo<ActiveChip[]>(() => {
    ...
    const pushMulti = (paramKey: string, values: string[]) => {
      for (const v of values) {
        out.push({
          id: `${paramKey}:${v}`,
          label: finFacetOptionLabel(paramKey, v),
          onRemove: () => state.toggleMulti(paramKey, v),
        })
      }
    }
    pushMulti(FIN_FACET_PARAM_KEYS.finSetup, state.selections.finSetups)
    pushMulti(FIN_FACET_PARAM_KEYS.finSystem, state.selections.finSystems)
    pushMulti(FIN_FACET_PARAM_KEYS.size, state.selections.sizes)
    pushMulti(FIN_FACET_PARAM_KEYS.condition, state.selections.conditions)
