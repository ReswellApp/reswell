...
import type { AccessoriesFilterState } from "@/components/accessories-browse-filter-state"
  ...
  return (
    <div className="flex flex-col">
      {options.map((opt) => (
        <CheckRow
          key={opt.value}
          id={`accessories-facet-${paramKey}-${opt.value}`}
          label={opt.label}
...
function PriceSection({ state }: { state: AccessoriesFilterState }) {
...
function BrandSection({ state }: { state: AccessoriesFilterState }) {
...
export function AccessoriesBrowseFacetControls({
  state,
  counts = {},
}: {
  state: AccessoriesFilterState
    ...
    <Accordion type="multiple" defaultValue={sectionIds} className="w-full">
      {hasSizeOptions ? (
        <FacetAccordionItem id="size" title="Size">
          <MultiSelectSection
            paramKey={ACCESSORY_FACET_PARAM_KEYS.size}
            options={ACCESSORY_SIZE_OPTIONS}
            selected={selections.sizes}
            counts={counts[ACCESSORY_FACET_PARAM_KEYS.size]}
            state={state}
          />
        </FacetAccordionItem>
      ) : null}
      ...
      <FacetAccordionItem id="condition" title="Condition">
        <MultiSelectSection
          paramKey={ACCESSORY_FACET_PARAM_KEYS.condition}
          options={ACCESSORY_CONDITION_OPTIONS}
          selected={selections.conditions}
          counts={counts[ACCESSORY_FACET_PARAM_KEYS.condition]}
          state={state}
        />
      </FacetAccordionItem>
