...
import { useTractionBrowseRouter } from "@/hooks/use-traction-browse-router"
import {
  TRACTION_FACET_PARAM_KEYS,
  tractionFacetSelectionsFromParams,
  hasAnyTractionFacetSelection,
  type TractionBrowseFacetSelections,
} from "@/lib/traction-browse-facets"
import {
...
const FACET_OWNED_KEYS = [
  ...Object.values(TRACTION_FACET_PARAM_KEYS),
  "brand",
  "minPrice",
  "maxPrice",
] as const

export type TractionFilterState = {
  searchParams: URLSearchParams
  selections: TractionBrowseFacetSelections
  brand: string
  minPrice: string
  maxPrice: string
  activeCount: number
...
export function useTractionFilterState(
  transitionStart?: (cb: () => void) => void,
): TractionFilterState {
  const { navigate, searchParams } = useTractionBrowseRouter(transitionStart)

  const selections = useMemo(
    () =>
      tractionFacetSelectionsFromParams({
        size: searchParams.get(TRACTION_FACET_PARAM_KEYS.size) ?? undefined,
        condition: searchParams.get(TRACTION_FACET_PARAM_KEYS.condition) ?? undefined,
      }),
    [searchParams],
  )
  ...
  const facetActive = hasAnyTractionFacetSelection(selections)
  const hasAnyActive =
