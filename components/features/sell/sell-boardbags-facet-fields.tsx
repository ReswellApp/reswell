...
function SellBoardbagFacetSelect({
  id,
  label,
  hint,
  value,
...
export type SellBoardbagsFacetFieldsProps = {
  condition: string
  size: string
  brand: string
  model: string
...
export function SellBoardbagsFacetFields({
  condition,
  size,
  brand,
  model,
  ...
  onBrandChange,
  onModelChange,
}: SellBoardbagsFacetFieldsProps) {
  const hasSizeOptions = BOARDBAG_SIZE_OPTIONS.length > 0
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <SellBoardbagFacetSelect
          id="sell-boardbag-condition"
          label="Condition"
          value={condition}
