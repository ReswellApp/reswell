...
import { FollowersTabs } from "@/app/dashboard/followers/followers-tabs"
...
export type FollowersDashboardPanelsProps = {
  variant?: "page" | "embedded" | "tab"
}

export async function FollowersDashboardPanels({ variant = "page" }: FollowersDashboardPanelsProps) {
  const { supabase, user } = await getCachedDashboardSession()

  const loginRedirect =
    variant === "embedded" ? "/dashboard/profile" : "/dashboard/followers"
  ...
  const tabs = <FollowersTabs followingPanel={followingPanel} followersPanel={followersPanel} />

  if (variant === "tab") {
    return (
      <div id="followers" className="scroll-mt-24 space-y-6">
        {tabs}
      </div>
    )
  }
  ...
  if (variant === "embedded") {
    return (
      <Card id="followers" className="scroll-mt-24">
        <CardHeader>
          <CardTitle>Followers &amp; following</CardTitle>
          <CardDescription>Sellers you follow and people following your shop</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">{tabs}</CardContent>
      </Card>
    )
  }
