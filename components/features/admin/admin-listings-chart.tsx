...
export interface MonthlyListingPoint {
  month: string
  label: string
  count: number
}

interface AdminListingsChartProps {
  data: MonthlyListingPoint[]
  className?: string
}
...
function ChartTooltip({ active, payload }: TooltipProps<number, string>) {
  ...
  const point = payload[0]?.payload as MonthlyListingPoint | undefined
...
export function AdminListingsChart({ data, className }: AdminListingsChartProps) {
      ...
      <div className="mt-4 h-[240px] w-full">
        {!hasData ? (
          <div className="flex h-full items-center justify-center rounded-lg border border-dashed border-border">
            <p className="text-sm text-muted-foreground">No listings in this window yet.</p>
          </div>
        ) : (
