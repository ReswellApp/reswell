...
import { TractionBrowseFacetControls } from "@/components/traction-browse-facet-controls"
import { useTractionFilterState } from "@/components/traction-browse-filter-state"
import { TRACTION_FACET_PARAM_KEYS, tractionFacetOptionLabel } from "@/lib/traction-browse-facets"
import { cn } from "@/lib/utils"

type ActiveChip = { id: string; label: string; onRemove: () => void }

export function TractionBrowseClient({
  children,
  title,
  description,
}: {
  ...
  const state = useTractionFilterState(startTransition)
  ...
  const chips = useMemo<ActiveChip[]>(() => {
    ...
    const pushMulti = (paramKey: string, values: string[]) => {
      for (const v of values) {
        out.push({
          id: `${paramKey}:${v}`,
          label: tractionFacetOptionLabel(paramKey, v),
          onRemove: () => state.toggleMulti(paramKey, v),
        })
      }
    }
    pushMulti(TRACTION_FACET_PARAM_KEYS.size, state.selections.sizes)
    pushMulti(TRACTION_FACET_PARAM_KEYS.condition, state.selections.conditions)
      ...
      <div className="mt-5 flex w-full min-w-0 gap-6">
        <aside className={cn("hidden w-[260px] shrink-0", desktopFiltersOpen && "md:block")}>
          <div className="sticky top-4">
            ...
            <TractionBrowseFacetControls state={state} />
          </div>
        </aside>
