...
import { TRACTION_SECTION, tractionSizeLabel } from "@/lib/traction-listing-config"
...
const SELLER_TRACTION_PDP_LIMIT = 12

export async function TractionListingDetailPage(props: ListingDetailPageSharedProps) {
  return renderListingDetailWithGuestFallback(props, renderTractionListingDetailPage)
}

async function renderTractionListingDetailPage({
  listingParam,
  prefetchedListing,
  viewerUser,
  ...
  const sellerId = traction.user_id as string
  const isSold = traction.status === "sold"
  const tractionHref = listingDetailHref({ id: traction.id as string, slug: traction.slug as string | null })

  const brandId = (traction.brand_id as string | null)?.trim() ?? ""

  // Wave 1: everything that depends only on the listing row runs in parallel.
  const listPriceNum =
    typeof traction.price === "number" ? traction.price : Number.parseFloat(String(traction.price)) || 0
  ...
  const [
    ...
    supabase
      ...
      .limit(SELLER_TRACTION_PDP_LIMIT),
    brandId ? getBrandById(supabase, brandId) : Promise.resolve(null),
    fetchSimilarPeerListingsForListingPdp(supabase, {
      excludeListingId: traction.id as string,
      section: TRACTION_SECTION,
      priceUsd: listPriceNum,
      facet: { column: "traction_size", value: traction.traction_size as string | null },
    }),
    Promise.all([
      !isSold ? getListingCartHolderCount(supabase, traction.id) : Promise.resolve(0),
      !isSold ? getListingFavoriteCount(supabase, traction.id) : Promise.resolve(0),
    ]),
  ])
  ...
  const sellerTraction = hydrateHomePeerListingRows((sellerTractionRes.data ?? []) as Record<string, unknown>[])
  const similarTraction = hydrateHomePeerListingRows(similarTractionRaw)

  const sellerTractionIds = (sellerTraction ?? []).map((f) => f.id)
  const similarTractionIds = similarTraction.map((r) => String(r.id))
  ...
  const [favoriteRowsRes, acceptedOffer] = await Promise.all([
    user
      ? supabase
          ...
          .select("listing_id")
          .eq("user_id", user.id)
          .in("listing_id", [traction.id, ...sellerTractionIds, ...similarTractionIds])
      : Promise.resolve({ data: null }),
    user && user.id !== traction.user_id && traction.status === "active"
      ? fetchAcceptedOfferForBuyerListing(supabase, user.id, traction.id)
  ...
  const isFavorited = favoritedIds.has(traction.id)
  const sellerTractionFavoritedIds = sellerTractionIds.filter((id) => favoritedIds.has(id))
  const similarTractionFavoritedIds = similarTractionIds.filter((id) => favoritedIds.has(id))

  const images = orderedListingGalleryImages(traction.listing_images as GalleryImage[] | null)
  ...
  const isOwnListing = user?.id === traction.user_id

  const pickupOffered = traction.local_pickup !== false
  const shippingOffered = !!traction.shipping_available

  const purchaseVisibility = {
    status: String(traction.status ?? ""),
    title: traction.title as string | null | undefined,
    hidden_from_site: traction.hidden_from_site as boolean | null | undefined,
    archived_at: traction.archived_at as string | null | undefined,
  }
  ...
  const freeBrandLabel = (traction.brand as string | null)?.trim() ?? ""
  ...
  const modelForSpecs = (traction.model as string | null)?.trim() || null
  const modelPagePath = await resolveListingModelPageHref(supabase, {
    brand: indexBrand,
    brandModelId: (traction.brand_model_id as string | null),
    modelName: modelForSpecs,
  })

  const sizeLabel = tractionSizeLabel(traction.traction_size as string | null)

  const listingTitle = capitalizeWords(traction.title as string)

  const publicListPriceUsd = publicListingListPriceUsd(traction.price)
  const compareAtPriceUsd = publicListingCompareAtPriceUsd(
    (traction as { compare_at_price?: string | number | null }).compare_at_price,
    listPriceNum,
  )
  const buyerOffersOn = (traction.buyer_offers_enabled as boolean | null) !== false
  const offerPct = effectiveMinimumOfferPct(traction as { minimum_offer_pct?: number | null })
  ...
  const shippingFlatRate = Math.max(0, Number.parseFloat(String(traction.shipping_price ?? 0)) || 0)
  ...
  if (acceptedOffer && acceptedOffer.seller_id === traction.user_id) {
  ...
  const listingLocationLine =
    traction.city && traction.state
      ? `${traction.city}, ${traction.state}`
      : (traction.profiles as { location?: string | null } | null)?.location?.trim() || null
  ...
  const listingViews = Number((traction.views as number | null) ?? 0)
  let listedRelative: string | null = null
  if (traction.created_at != null) {
    const d = new Date(traction.created_at as string | number | Date)
  ...
  const aboutSellerSection = (
    <ListingAboutSellerSection
      profiles={traction.profiles as AboutSellerProfilesProp}
      listingImageFallbacks={[{ listing_images: traction.listing_images }]}
      sellerProfileHref={sellerProfileHref(traction.profiles)}
      messageHrefAuthenticated={`/messages/new?user=${traction.user_id}&listing=${traction.id}`}
      messageHrefLoginRedirect={`/auth/login?redirect=${encodeURIComponent(tractionHref)}`}
      isLoggedIn={!!user}
      isOwnListing={isOwnListing}
      isSold={isSold}
      avgRating={sellerAvgRating}
      reviewCount={sellerReviewCount}
      itemsSold={Number((traction.profiles as { sales_count?: number } | null)?.sales_count ?? 0)}
  ...
  return (
    <main className="relative flex-1 w-full min-w-0 max-w-full overflow-x-clip bg-background pb-16 pt-2 sm:pb-24 sm:pt-3 lg:pt-8">
      <div className="container mx-auto w-full min-w-0 max-w-full px-4 sm:px-6 lg:px-8 lg:!max-w-[min(100%,1320px)] xl:!max-w-[min(100%,1480px)] 2xl:!max-w-[min(100%,1680px)]">
        <div className="mb-3 min-w-0 max-w-full pt-0.5 max-lg:mb-4 lg:mb-8">
          <Breadcrumb>
            <BreadcrumbList className="gap-1 text-[13px] font-normal tracking-wide text-muted-foreground sm:gap-1.5 sm:text-[14px]">
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild className="transition-colors hover:text-foreground">
                  <Link href="/traction">Traction</Link>
                </BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator className="text-muted-foreground/70 [&>svg]:stroke-[1.25]" />
              <BreadcrumbItem>
        ...
        <div className="mx-auto grid w-full min-w-0 max-w-full gap-x-8 gap-y-2 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] lg:grid-rows-[auto_auto_auto] lg:[grid-template-areas:'gallery_details'_'about_details'_'similar_similar'] lg:items-start lg:gap-x-12 lg:gap-y-0 xl:gap-x-16">
          ...
          <div className="min-w-0 max-w-full max-lg:order-2 lg:hidden">
            <ListingMobileBuySummary
              ...
              showScarcity={canPeerPurchase && traction.status === "active"}
              views={listingViews}
              watchers={listingWatchersCount}
              cartHolderCount={cartHolderCount}
              offerToCart={
                isOwnListing && user
                  ? { listingId: traction.id, sellerUserId: user.id, listingTitle, listPrice: listPriceNum }
                  : null
              }
              createdAt={traction.created_at}
            ...
            >
              {canPeerPurchase ? (
                <ListingDetailPeerPurchaseActionsLoader
                  listingId={traction.id}
                  checkoutListingParam={traction.slug ?? traction.id}
                  section="traction"
                  isLoggedIn={!!user}
          ...
          <div className="min-w-0 space-y-5 max-lg:order-4 lg:[grid-area:details] lg:order-none lg:pt-1">
            <div className="hidden lg:block">
              ...
              {!isSold && !isOwnListing && listingPurchasable && traction.status === "active" ? (
            ...
            {!isOwnListing ? (
              <div className="border-b border-neutral-200/90 pb-4 dark:border-neutral-700/70">
                <Link
                  href={`/contact?topic=listing-report&listing=${encodeURIComponent(traction.id)}`}
            ...
            {isOwnListing ? (
              <ListingOwnerManageActions
                ...
                listingStatus={String(traction.status ?? "")}
                hiddenFromSite={traction.hidden_from_site === true}
              />
            ) : null}
          </div>
                ...
                <AccordionContent className="pb-6 pt-0">
                  <div className="text-[16px] leading-[1.65] text-foreground">
                    <TranslateableDescription
                      text={(traction.description as string) || ""}
              ...
              {!isOwnListing && !isSold ? (
                <AccordionItem value="contact" className="border-border/55">
                  ...
                  <AccordionContent className="pb-6 pt-0">
                    <ContactSellerForm
                      listingId={traction.id}
                      listingSlug={traction.slug}
                      sellerId={traction.user_id}
            ...
            {similarTraction.length > 0 ? (
              <div className="col-span-full min-w-0 max-w-full max-lg:order-5 lg:[grid-area:similar] lg:order-none">
                <section className="mt-10 border-t border-neutral-200/90 pt-8 dark:border-neutral-700/70">
                  <h2 className="mb-8 text-2xl font-bold text-foreground">Similar traction</h2>
                  <HomeListingScrollRow uniformCardHeights>
                    {similarTraction.map((row) => (
                      <HomePeerListingScrollTile
                        key={String(row.id)}
                        listing={row as unknown as HomePeerScrollListing}
                        userId={user?.id ?? null}
                        isFavorited={similarTractionFavoritedIds.includes(String(row.id))}
                      />
                    ))}
                  </HomeListingScrollRow>
                </section>
        ...
        <ListingRelatedContentSection listingId={traction.id as string} variant="embedded" />

        {sellerTraction && sellerTraction.length > 0 && (
          <section className="mt-16 min-w-0 w-full border-t border-neutral-200/90 pt-12 dark:border-neutral-700/70">
            <h2 className="mb-8 text-2xl font-bold text-foreground">More traction from this seller</h2>
            <HomeListingScrollRow uniformCardHeights>
              {sellerTraction.map((item) => (
                <HomePeerListingScrollTile
                  key={item.id}
                  listing={item}
                  userId={user?.id ?? null}
                  isFavorited={sellerTractionFavoritedIds.includes(item.id)}
                />
              ))}
            </HomeListingScrollRow>
          </section>
