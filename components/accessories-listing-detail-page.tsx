...
async function renderAccessoriesListingDetailPage({
  ...
  const sellerAccessories = hydrateHomePeerListingRows((sellerAccessoriesRes.data ?? []) as Record<string, unknown>[])
  ...
  const modelPagePath = await resolveListingModelPageHref(supabase, {
    brand: indexBrand,
    brandModelId: (accessory.brand_model_id as string | null),
  ...
  const compareAtPriceUsd = publicListingCompareAtPriceUsd(
    (accessory as { compare_at_price?: string | number | null }).compare_at_price,
    listPriceNum,
  )
        ...
        <div className="mx-auto grid w-full min-w-0 max-w-full gap-x-8 gap-y-2 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] lg:grid-rows-[auto_auto_auto] lg:[grid-template-areas:'gallery_details'_'about_details'_'similar_similar'] lg:items-start lg:gap-x-12 lg:gap-y-0 xl:gap-x-16">
          ...
          <div className="min-w-0 max-w-full max-lg:order-2 lg:hidden">
            <ListingMobileBuySummary
              ...
              offerToCart={
                isOwnListing && user
                  ? { listingId: accessory.id, sellerUserId: user.id, listingTitle, listPrice: listPriceNum }
                  : null
              }
