...
export async function AccessoriesListingDetailPage({
  ...
  const sellerId = accessory.user_id as string
  const isSold = accessory.status === "sold"
  const accessoryHref = listingDetailHref({ id: accessory.id as string, slug: accessory.slug as string | null })

  const brandId = (accessory.brand_id as string | null)?.trim() ?? ""
  ...
  const [
    ...
    Promise.all([
      ...
      !isSold ? getListingFavoriteCount(supabase, accessory.id) : Promise.resolve(0),
    ]),
  ])
  ...
  const [favoriteRowsRes, acceptedOffer] = await Promise.all([
    user
      ? supabase
          ...
          .select("listing_id")
          .eq("user_id", user.id)
          .in("listing_id", [accessory.id, ...sellerAccessoryIds])
      : Promise.resolve({ data: null }),
    user && user.id !== accessory.user_id && accessory.status === "active"
      ? fetchAcceptedOfferForBuyerListing(supabase, user.id, accessory.id)
  ...
  const isFavorited = favoritedIds.has(accessory.id)
  const sellerAccessoryFavoritedIds = sellerAccessoryIds.filter((id) => favoritedIds.has(id))

  const images: GalleryImage[] =
    (accessory.listing_images as GalleryImage[] | null)
      ?.slice()
      .sort(
        (a, b) =>
  ...
  const canPeerPurchase = peerListingCanPurchase(listingInventoryFieldsFromRow(accessory as Record<string, unknown>), { isOwnListing })
  ...
  const modelForSpecs = (accessory.model as string | null)?.trim() || null

  const sizeLabel = accessorySizeLabel(accessory.accessory_size as string | null)

  const listingTitle = capitalizeWords(accessory.title as string)

  const listPriceNum =
    typeof accessory.price === "number" ? accessory.price : Number.parseFloat(String(accessory.price)) || 0
  const publicListPriceUsd = publicListingListPriceUsd(accessory.price)
  const buyerOffersOn = (accessory.buyer_offers_enabled as boolean | null) !== false
  const offerPct = effectiveMinimumOfferPct(accessory as { minimum_offer_pct?: number | null })
  ...
  const shippingFlatRate = Math.max(0, Number.parseFloat(String(accessory.shipping_price ?? 0)) || 0)
  ...
  if (acceptedOffer && acceptedOffer.seller_id === accessory.user_id) {
  ...
  const listingLocationLine =
    accessory.city && accessory.state
      ? `${accessory.city}, ${accessory.state}`
      : (accessory.profiles as { location?: string | null } | null)?.location?.trim() || null
  ...
  const listingViews = Number((accessory.views as number | null) ?? 0)
  let listedRelative: string | null = null
  if (accessory.created_at != null) {
    const d = new Date(accessory.created_at as string | number | Date)
  ...
  const aboutSellerSection = (
    <ListingAboutSellerSection
      profiles={accessory.profiles as AboutSellerProfilesProp}
      listingImageFallbacks={[{ listing_images: accessory.listing_images }]}
      sellerProfileHref={sellerProfileHref(accessory.profiles)}
      messageHrefAuthenticated={`/messages/new?user=${accessory.user_id}&listing=${accessory.id}`}
      messageHrefLoginRedirect={`/auth/login?redirect=${encodeURIComponent(accessoryHref)}`}
      isLoggedIn={!!user}
      isOwnListing={isOwnListing}
      isSold={isSold}
      avgRating={sellerAvgRating}
      reviewCount={sellerReviewCount}
      itemsSold={Number((accessory.profiles as { sales_count?: number } | null)?.sales_count ?? 0)}
      ...
      <div className="container mx-auto w-full min-w-0 max-w-full px-4 sm:px-6 lg:px-8 lg:!max-w-[min(100%,1320px)] xl:!max-w-[min(100%,1480px)] 2xl:!max-w-[min(100%,1680px)]">
        ...
        <div className="mx-auto grid w-full min-w-0 max-w-full gap-x-8 gap-y-5 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:grid-rows-[auto_auto_auto] lg:[grid-template-areas:'gallery_details'_'about_details'_'similar_similar'] lg:items-start lg:gap-x-12 lg:gap-y-0 xl:gap-x-16">
          ...
          <div className="min-w-0 max-w-full max-lg:order-2 lg:hidden">
            ...
            {canPeerPurchase ? (
              <ListingStockAvailabilityMessage listing={listingInventoryFieldsFromRow(accessory as Record<string, unknown>)} className="mt-3" />
            ) : null}
            {canPeerPurchase ? (
              <div className="mt-5">
          ...
          <div className="min-w-0 space-y-5 max-lg:order-4 lg:[grid-area:details] lg:order-none lg:pt-1">
            <div className="hidden lg:block">
              ...
              {canPeerPurchase ? (
                <ListingStockAvailabilityMessage listing={listingInventoryFieldsFromRow(accessory as Record<string, unknown>)} className="mt-4" />
              ) : null}
              {!isSold && !isOwnListing ? (
            ...
            {!isOwnListing ? (
              <div className="border-b border-neutral-200/90 pb-4 dark:border-neutral-700/70">
                <Link
                  href={`/contact?topic=listing-report&listing=${encodeURIComponent(accessory.id)}`}
                ...
                <AccordionContent className="pb-6 pt-0">
                  <div className="text-[16px] leading-[1.65] text-foreground">
                    <TranslateableDescription
                      text={(accessory.description as string) || ""}
                ...
                <AccordionContent className="pb-6 pt-0">
                  <div className="space-y-3 text-[16px] leading-[1.65] text-foreground">
                    ...
                    <p>
                      ...
                      {pickupOffered && !shippingOffered &&
                        "Local pickup only — meet the seller near this area to inspect the accessory."}
                      {!pickupOffered && shippingOffered &&
              ...
              {!isOwnListing && !isSold ? (
                <AccordionItem value="contact" className="border-border/55">
                  ...
                  <AccordionContent className="pb-6 pt-0">
                    <ContactSellerForm
                      listingId={accessory.id}
                      listingSlug={accessory.slug}
                      sellerId={accessory.user_id}
        ...
        {sellerAccessories && sellerAccessories.length > 0 && (
          <section className="mt-16 min-w-0 w-full border-t border-neutral-200/90 pt-12 dark:border-neutral-700/70">
            <h2 className="mb-8 text-2xl font-bold text-foreground">More accessories from this seller</h2>
            <HomeListingScrollRow uniformCardHeights>
              {sellerAccessories.map((item) => (
                <HomePeerListingScrollTile
                  key={item.id}
                  listing={item}
                  userId={user?.id ?? null}
                  isFavorited={sellerAccessoryFavoritedIds.includes(item.id)}
                />
              ))}
            </HomeListingScrollRow>
          </section>
