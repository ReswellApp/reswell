...
import {
  ...
  boardbagFacetSelectionsFromParams,
  hasAnyBoardbagFacetSelection,
  type BoardbagsBrowseFacetSelections,
...
const FACET_OWNED_KEYS = [
  ...Object.values(BOARDBAG_FACET_PARAM_KEYS),
  "brand",
  "minPrice",
  "maxPrice",
] as const

export type BoardbagsFilterState = {
  searchParams: URLSearchParams
  selections: BoardbagsBrowseFacetSelections
  brand: string
  minPrice: string
  maxPrice: string
  activeCount: number
...
export function useBoardbagsFilterState(
  transitionStart?: (cb: () => void) => void,
): BoardbagsFilterState {
  const { navigate, searchParams } = useBoardbagsBrowseRouter(transitionStart)

  const selections = useMemo(
    () =>
      boardbagFacetSelectionsFromParams({
        size: searchParams.get(BOARDBAG_FACET_PARAM_KEYS.size) ?? undefined,
        condition: searchParams.get(BOARDBAG_FACET_PARAM_KEYS.condition) ?? undefined,
      }),
    [searchParams],
  )
  ...
  const facetActive = hasAnyBoardbagFacetSelection(selections)
  const hasAnyActive =
