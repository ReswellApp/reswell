...
export async function SurfpacksListingDetailPage(props: ListingDetailPageSharedProps) {
  return renderListingDetailWithGuestFallback(props, renderSurfpacksListingDetailPage)
}

async function renderSurfpacksListingDetailPage({
  listingParam,
  prefetchedListing,
  viewerUser,
  ...
  const [
    ...
    fetchSimilarPeerListingsForListingPdp(supabase, {
      excludeListingId: surfpack.id as string,
      section: SURFPACKS_SECTION,
      priceUsd: listPriceNum,
      facet: { column: "surfpack_size", value: surfpack.surfpack_size as string | null },
    }),
    Promise.all([
  ...
  const similarSurfpacks = hydrateHomePeerListingRows(similarSurfpacksRaw)

  const sellerSurfpackIds = (sellerSurfpacks ?? []).map((f) => f.id)
  const similarSurfpackIds = similarSurfpacks.map((r) => String(r.id))
  ...
  const [favoriteRowsRes, acceptedOffer] = await Promise.all([
    user
      ? supabase
          ...
          .select("listing_id")
          .eq("user_id", user.id)
          .in("listing_id", [surfpack.id, ...sellerSurfpackIds, ...similarSurfpackIds])
  ...
  const similarSurfpackFavoritedIds = similarSurfpackIds.filter((id) => favoritedIds.has(id))

  const images = orderedListingGalleryImages(surfpack.listing_images as GalleryImage[] | null)
  ...
  const purchaseVisibility = {
    status: String(surfpack.status ?? ""),
    title: surfpack.title as string | null | undefined,
    hidden_from_site: surfpack.hidden_from_site as boolean | null | undefined,
    archived_at: surfpack.archived_at as string | null | undefined,
  }
  ...
  const compareAtPriceUsd = publicListingCompareAtPriceUsd(
    (surfpack as { compare_at_price?: string | number | null }).compare_at_price,
    listPriceNum,
  )
  ...
  const specRows = [
    listingConditionSpecRow(surfpack.condition as string | null),
  ...
  return (
    <main className="relative flex-1 w-full min-w-0 max-w-full overflow-x-clip bg-background pb-16 pt-2 sm:pb-24 sm:pt-3 lg:pt-8">
      <div className="container mx-auto w-full min-w-0 max-w-full px-4 sm:px-6 lg:px-8 lg:!max-w-[min(100%,1320px)] xl:!max-w-[min(100%,1480px)] 2xl:!max-w-[min(100%,1680px)]">
        ...
        <div className="mx-auto grid w-full min-w-0 max-w-full gap-x-8 gap-y-2 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] lg:grid-rows-[auto_auto_auto] lg:[grid-template-areas:'gallery_details'_'about_details'_'similar_similar'] lg:items-start lg:gap-x-12 lg:gap-y-0 xl:gap-x-16">
          ...
          <div className="min-w-0 max-w-full max-lg:order-2 lg:hidden">
            <ListingMobileBuySummary
              ...
              showScarcity={canPeerPurchase && surfpack.status === "active"}
              views={listingViews}
              watchers={listingWatchersCount}
              cartHolderCount={cartHolderCount}
              offerToCart={
                isOwnListing && user
                  ? { listingId: surfpack.id, sellerUserId: user.id, listingTitle, listPrice: listPriceNum }
                  : null
              }
              createdAt={surfpack.created_at}
          ...
          <div className="min-w-0 space-y-5 max-lg:order-4 lg:[grid-area:details] lg:order-none lg:pt-1">
            <div className="hidden lg:block">
              ...
              {!isSold && !isOwnListing && listingPurchasable && surfpack.status === "active" ? (
            ...
            {similarSurfpacks.length > 0 ? (
              <div className="col-span-full min-w-0 max-w-full max-lg:order-5 lg:[grid-area:similar] lg:order-none">
                <section className="mt-10 border-t border-neutral-200/90 pt-8 dark:border-neutral-700/70">
                  <h2 className="mb-8 text-2xl font-bold text-foreground">Similar surfpacks</h2>
                  <HomeListingScrollRow uniformCardHeights>
                    {similarSurfpacks.map((row) => (
                      <HomePeerListingScrollTile
                        key={String(row.id)}
                        listing={row as unknown as HomePeerScrollListing}
                        userId={user?.id ?? null}
                        isFavorited={similarSurfpackFavoritedIds.includes(String(row.id))}
                      />
                    ))}
                  </HomeListingScrollRow>
                </section>
        ...
        <ListingRelatedContentSection listingId={surfpack.id as string} variant="embedded" />
