...
import { ApparelBrowseFilterToolbar } from "@/components/apparel-browse-filter-toolbar"
import { ApparelBrowseFacetControls } from "@/components/apparel-browse-facet-controls"
...
export function ApparelBrowseClient({ children }: { children: ReactNode }) {
  const [isPending, startTransition] = useTransition()
  const [mobileOpen, setMobileOpen] = useState(false)
  const [desktopFiltersOpen, setDesktopFiltersOpen] = useState(false)
  const state = useApparelFilterState(startTransition)
  ...
  const chips = useMemo<ActiveChip[]>(() => {
    ...
    const pushMulti = (paramKey: string, values: string[]) => {
      for (const v of values) {
        out.push({
          id: `${paramKey}:${v}`,
          label: apparelFacetOptionLabel(paramKey, v),
      ...
      <div className="mt-4 flex w-full min-w-0 gap-6">
        <aside className={cn("hidden w-[260px] shrink-0", desktopFiltersOpen && "md:block")}>
          <div className="sticky top-4">
            ...
            <ApparelBrowseFacetControls state={state} />
          </div>
        </aside>
