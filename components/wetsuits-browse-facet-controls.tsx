...
import type { WetsuitsFilterState } from "@/components/wetsuits-browse-filter-state"
  ...
  return (
    <div className="flex flex-col">
      {options.map((opt) => (
        <CheckRow
          key={opt.value}
          id={`wetsuits-facet-${paramKey}-${opt.value}`}
          label={opt.label}
...
function PriceSection({ state }: { state: WetsuitsFilterState }) {
...
function BrandSection({ state }: { state: WetsuitsFilterState }) {
...
export function WetsuitsBrowseFacetControls({
  state,
  counts = {},
}: {
  state: WetsuitsFilterState
    ...
    <Accordion type="multiple" defaultValue={sectionIds} className="w-full">
      {hasSizeOptions ? (
        <FacetAccordionItem id="size" title="Size">
          <MultiSelectSection
            paramKey={WETSUIT_FACET_PARAM_KEYS.size}
            options={WETSUIT_SIZE_OPTIONS}
            selected={selections.sizes}
            counts={counts[WETSUIT_FACET_PARAM_KEYS.size]}
            state={state}
          />
        </FacetAccordionItem>
      ) : null}
      ...
      <FacetAccordionItem id="condition" title="Condition">
        <MultiSelectSection
          paramKey={WETSUIT_FACET_PARAM_KEYS.condition}
          options={WETSUIT_CONDITION_OPTIONS}
          selected={selections.conditions}
          counts={counts[WETSUIT_FACET_PARAM_KEYS.condition]}
          state={state}
        />
      </FacetAccordionItem>
