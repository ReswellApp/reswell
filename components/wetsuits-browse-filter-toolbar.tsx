...
import { WETSUITS_BROWSE_DEFAULT_SORT } from "@/lib/wetsuits-browse-metadata"

export const wetsuitSortOptions = [
  { value: WETSUITS_BROWSE_DEFAULT_SORT, label: "Newest" },
...
/** Slim browse toolbar for `/wetsuits`: keyword search, sort, plus Filter triggers. */
export function WetsuitsBrowseFilterToolbar({
  resultCount,
  activeFilterCount,
  onOpenMobileFilters,
  ...
  const sort = searchParams.get("sort") ?? WETSUITS_BROWSE_DEFAULT_SORT
  ...
  const sortSelect = (
    <Select
      ...
      value={sort}
      onValueChange={(v) =>
        navigate((p) => (v === WETSUITS_BROWSE_DEFAULT_SORT ? p.delete("sort") : p.set("sort", v)))
      }
    >
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-3 md:flex-row md:items-center md:gap-3">
        ...
        {resultCount != null ? (
          <span className="order-3 hidden shrink-0 text-sm text-muted-foreground md:order-4 lg:inline">
            {resultCount.toLocaleString()} {resultCount === 1 ? "wetsuit" : "wetsuits"}
          </span>
        ) : null}
      </div>
    </form>
