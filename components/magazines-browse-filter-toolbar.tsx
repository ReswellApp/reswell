...
import { MAGAZINES_BROWSE_DEFAULT_SORT } from "@/lib/magazines-browse-metadata"

export const magazineSortOptions = [
  { value: MAGAZINES_BROWSE_DEFAULT_SORT, label: "Newest" },
...
export function MagazinesBrowseFilterToolbar({
  resultCount,
  activeFilterCount,
  onOpenMobileFilters,
  ...
  const sort = searchParams.get("sort") ?? MAGAZINES_BROWSE_DEFAULT_SORT
  ...
  const sortSelect = (
    <Select
      ...
      value={sort}
      onValueChange={(v) =>
        navigate((p) => (v === MAGAZINES_BROWSE_DEFAULT_SORT ? p.delete("sort") : p.set("sort", v)))
      }
    >
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-3 md:flex-row md:items-center md:gap-3">
        ...
        {resultCount != null ? (
          <span className="order-3 hidden shrink-0 text-sm text-muted-foreground md:order-4 lg:inline">
            {resultCount.toLocaleString()} {resultCount === 1 ? "magazine" : "magazines"}
          </span>
        ) : null}
      </div>
    </form>
