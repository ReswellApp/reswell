...
const SELLER_WETSUITS_PDP_LIMIT = 12

export async function WetsuitsListingDetailPage(props: ListingDetailPageSharedProps) {
  return renderListingDetailWithGuestFallback(props, renderWetsuitsListingDetailPage)
}

async function renderWetsuitsListingDetailPage({
  listingParam,
  prefetchedListing,
  viewerUser,
  ...
  const sellerId = wetsuit.user_id as string
  ...
  const [
    ...
    fetchSimilarPeerListingsForListingPdp(supabase, {
      excludeListingId: wetsuit.id as string,
      section: WETSUITS_SECTION,
      priceUsd: listPriceNum,
      facet: { column: "wetsuit_size", value: wetsuit.wetsuit_size as string | null },
    }),
    Promise.all([
  ...
  const sellerWetsuits = hydrateHomePeerListingRows((sellerWetsuitsRes.data ?? []) as Record<string, unknown>[])
  const similarWetsuits = hydrateHomePeerListingRows(similarWetsuitsRaw)

  const sellerWetsuitIds = (sellerWetsuits ?? []).map((f) => f.id)
  const similarWetsuitIds = similarWetsuits.map((r) => String(r.id))
  ...
  const [favoriteRowsRes, acceptedOffer] = await Promise.all([
    user
      ? supabase
          ...
          .select("listing_id")
          .eq("user_id", user.id)
          .in("listing_id", [wetsuit.id, ...sellerWetsuitIds, ...similarWetsuitIds])
  ...
  const similarWetsuitFavoritedIds = similarWetsuitIds.filter((id) => favoritedIds.has(id))

  const images = orderedListingGalleryImages(wetsuit.listing_images as GalleryImage[] | null)
  ...
  const isOwnListing = user?.id === wetsuit.user_id
  ...
  const modelForSpecs = (wetsuit.model as string | null)?.trim() || null
  const modelPagePath = await resolveListingModelPageHref(supabase, {
    brand: indexBrand,
    brandModelId: (wetsuit.brand_model_id as string | null),
  ...
  const shippingFlatRate = Math.max(0, Number.parseFloat(String(wetsuit.shipping_price ?? 0)) || 0)
  ...
  const listingLocationLine =
    wetsuit.city && wetsuit.state
      ? `${wetsuit.city}, ${wetsuit.state}`
      : (wetsuit.profiles as { location?: string | null } | null)?.location?.trim() || null
  ...
  const aboutSellerSection = (
    <ListingAboutSellerSection
      profiles={wetsuit.profiles as AboutSellerProfilesProp}
      listingImageFallbacks={[{ listing_images: wetsuit.listing_images }]}
      sellerProfileHref={sellerProfileHref(wetsuit.profiles)}
      messageHrefAuthenticated={`/messages/new?user=${wetsuit.user_id}&listing=${wetsuit.id}`}
      messageHrefLoginRedirect={`/auth/login?redirect=${encodeURIComponent(wetsuitHref)}`}
      isLoggedIn={!!user}
      isOwnListing={isOwnListing}
      isSold={isSold}
      avgRating={sellerAvgRating}
      reviewCount={sellerReviewCount}
      itemsSold={Number((wetsuit.profiles as { sales_count?: number } | null)?.sales_count ?? 0)}
  ...
  return (
    <main className="relative flex-1 w-full min-w-0 max-w-full overflow-x-clip bg-background pb-16 pt-2 sm:pb-24 sm:pt-3 lg:pt-8">
      ...
      <div className="container mx-auto w-full min-w-0 max-w-full px-4 sm:px-6 lg:px-8 lg:!max-w-[min(100%,1320px)] xl:!max-w-[min(100%,1480px)] 2xl:!max-w-[min(100%,1680px)]">
        <div className="mb-3 min-w-0 max-w-full pt-0.5 max-lg:mb-4 lg:mb-8">
          <Breadcrumb>
            <BreadcrumbList className="gap-1 text-[13px] font-normal tracking-wide text-muted-foreground sm:gap-1.5 sm:text-[14px]">
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild className="transition-colors hover:text-foreground">
                  <Link href="/wetsuits">Wetsuits</Link>
                </BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator className="text-muted-foreground/70 [&>svg]:stroke-[1.25]" />
              <BreadcrumbItem>
          ...
          <div className="min-w-0 space-y-5 max-lg:order-4 lg:[grid-area:details] lg:order-none lg:pt-1">
            ...
            {!isOwnListing ? (
              <div className="border-b border-neutral-200/90 pb-4 dark:border-neutral-700/70">
                <Link
                  href={`/contact?topic=listing-report&listing=${encodeURIComponent(wetsuit.id)}`}
                ...
                <AccordionContent className="pb-6 pt-0">
                  <div className="text-[16px] leading-[1.65] text-foreground">
                    <TranslateableDescription
                      text={(wetsuit.description as string) || ""}
              ...
              {!isOwnListing && !isSold ? (
                <AccordionItem value="contact" className="border-border/55">
                  ...
                  <AccordionContent className="pb-6 pt-0">
                    <ContactSellerForm
                      listingId={wetsuit.id}
                      listingSlug={wetsuit.slug}
                      sellerId={wetsuit.user_id}
            ...
            {similarWetsuits.length > 0 ? (
              <div className="col-span-full min-w-0 max-w-full max-lg:order-5 lg:[grid-area:similar] lg:order-none">
                <section className="mt-10 border-t border-neutral-200/90 pt-8 dark:border-neutral-700/70">
                  <h2 className="mb-8 text-2xl font-bold text-foreground">Similar wetsuits</h2>
                  <HomeListingScrollRow uniformCardHeights>
                    {similarWetsuits.map((row) => (
                      <HomePeerListingScrollTile
                        key={String(row.id)}
                        listing={row as unknown as HomePeerScrollListing}
                        userId={user?.id ?? null}
                        isFavorited={similarWetsuitFavoritedIds.includes(String(row.id))}
                      />
                    ))}
                  </HomeListingScrollRow>
                </section>
        ...
        <ListingRelatedContentSection listingId={wetsuit.id as string} variant="embedded" />

        {sellerWetsuits && sellerWetsuits.length > 0 && (
          <section className="mt-16 min-w-0 w-full border-t border-neutral-200/90 pt-12 dark:border-neutral-700/70">
            <h2 className="mb-8 text-2xl font-bold text-foreground">More wetsuits from this seller</h2>
            <HomeListingScrollRow uniformCardHeights>
              {sellerWetsuits.map((item) => (
                <HomePeerListingScrollTile
                  key={item.id}
                  listing={item}
                  userId={user?.id ?? null}
                  isFavorited={sellerWetsuitFavoritedIds.includes(item.id)}
                />
              ))}
            </HomeListingScrollRow>
          </section>
