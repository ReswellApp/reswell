...
import { ACCESSORIES_BROWSE_DEFAULT_SORT } from "@/lib/accessories-browse-metadata"

export const accessorySortOptions = [
  { value: ACCESSORIES_BROWSE_DEFAULT_SORT, label: "Newest" },
...
/** Slim browse toolbar for `/accessories`: keyword search, sort, plus Filter triggers. */
export function AccessoriesBrowseFilterToolbar({
  resultCount,
  activeFilterCount,
  onOpenMobileFilters,
  ...
  const sort = searchParams.get("sort") ?? ACCESSORIES_BROWSE_DEFAULT_SORT
  ...
  const sortSelect = (
    <Select
      ...
      value={sort}
      onValueChange={(v) =>
        navigate((p) => (v === ACCESSORIES_BROWSE_DEFAULT_SORT ? p.delete("sort") : p.set("sort", v)))
      }
    >
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-3 md:flex-row md:items-center md:gap-3">
        ...
        {resultCount != null ? (
          <span className="order-3 hidden shrink-0 text-sm text-muted-foreground md:order-4 lg:inline">
            {resultCount.toLocaleString()} {resultCount === 1 ? "accessory" : "accessories"}
          </span>
        ) : null}
      </div>
    </form>
