...
import { useApparelBrowseRouter } from "@/hooks/use-apparel-browse-router"
...
import { APPAREL_BROWSE_DEFAULT_SORT } from "@/lib/apparel-browse-metadata"

export const apparelSortOptions = [
  { value: APPAREL_BROWSE_DEFAULT_SORT, label: "Newest" },
...
/** Slim browse toolbar for `/apparel`: keyword search, sort, plus Filter triggers. */
export function ApparelBrowseFilterToolbar({
  resultCount,
  activeFilterCount,
  onOpenMobileFilters,
  ...
  const { navigate, searchParams } = useApparelBrowseRouter(transitionStart)

  const urlQ = searchParams.get("q") ?? ""
  const [q, setQ] = useState(urlQ)
  const sort = searchParams.get("sort") ?? APPAREL_BROWSE_DEFAULT_SORT
  ...
  const sortSelect = (
    <Select
      ...
      value={sort}
      onValueChange={(v) =>
        navigate((p) => (v === APPAREL_BROWSE_DEFAULT_SORT ? p.delete("sort") : p.set("sort", v)))
      }
    >
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-3 md:flex-row md:items-center md:gap-3">
        ...
        {resultCount != null ? (
          <span className="order-3 hidden shrink-0 text-sm text-muted-foreground md:order-4 lg:inline">
            {resultCount.toLocaleString()} {resultCount === 1 ? "apparel" : "apparel"}
          </span>
        ) : null}
      </div>
    </form>
