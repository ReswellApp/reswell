...
const CLEAR_HREF = "/gear"

export async function UsedAllGearPage(props: {
  ...
  const categoryInMenu =
    !rawCategory || rawCategory === "all" || sorted.some((c) => c.slug === rawCategory)
  ...
  const selectedSlug =
    rawCategory && rawCategory !== "all" && sorted.some((c) => c.slug === rawCategory)
      ? rawCategory
      : null
  const selectedCategory = selectedSlug ? sorted.find((c) => c.slug === selectedSlug) : undefined
  const pageTitle = selectedCategory ? formatCategory(selectedCategory.name) : "All Gear"
    ...
    <main className="flex-1">
      <section className="bg-offwhite py-12">
        <div className="container mx-auto">
          ...
          <p className="text-center text-muted-foreground mt-2">{pageSubtitle}</p>
        </div>
      </section>
