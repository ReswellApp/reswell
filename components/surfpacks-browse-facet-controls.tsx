...
import type { SurfpacksFilterState } from "@/components/surfpacks-browse-filter-state"
  ...
  return (
    <div className="flex flex-col">
      {options.map((opt) => (
        <CheckRow
          key={opt.value}
          id={`surfpacks-facet-${paramKey}-${opt.value}`}
          label={opt.label}
...
function PriceSection({ state }: { state: SurfpacksFilterState }) {
...
function BrandSection({ state }: { state: SurfpacksFilterState }) {
...
export function SurfpacksBrowseFacetControls({
  state,
  counts = {},
}: {
  state: SurfpacksFilterState
    ...
    <Accordion type="multiple" defaultValue={sectionIds} className="w-full">
      {hasSizeOptions ? (
        <FacetAccordionItem id="size" title="Size">
          <MultiSelectSection
            paramKey={SURFPACK_FACET_PARAM_KEYS.size}
            options={SURFPACK_SIZE_OPTIONS}
            selected={selections.sizes}
            counts={counts[SURFPACK_FACET_PARAM_KEYS.size]}
            state={state}
          />
        </FacetAccordionItem>
      ) : null}
      ...
      <FacetAccordionItem id="condition" title="Condition">
        <MultiSelectSection
          paramKey={SURFPACK_FACET_PARAM_KEYS.condition}
          options={SURFPACK_CONDITION_OPTIONS}
          selected={selections.conditions}
          counts={counts[SURFPACK_FACET_PARAM_KEYS.condition]}
          state={state}
        />
      </FacetAccordionItem>
