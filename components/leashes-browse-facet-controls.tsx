...
import type { LeashesFilterState } from "@/components/leashes-browse-filter-state"
...
function MultiSelectSection({
  ...
  paramKey: string
  options: readonly LeashFacetOption[]
  selected: string[]
  counts: Record<string, number> | undefined
  state: LeashesFilterState
}) {
  return (
    <div className="flex flex-col">
      {options.map((opt) => (
        <CheckRow
          key={opt.value}
          id={`leashes-facet-${paramKey}-${opt.value}`}
          label={opt.label}
...
function PriceSection({ state }: { state: LeashesFilterState }) {
...
function BrandSection({ state }: { state: LeashesFilterState }) {
...
export function LeashesBrowseFacetControls({
  state,
  counts = {},
}: {
  state: LeashesFilterState
  ...
  const hasSizeOptions = LEASH_SIZE_OPTIONS.length > 0
  const sectionIds = [
    ...(hasSizeOptions ? ["size"] : []),
    "brand",
    "condition",
    ...
    <Accordion type="multiple" defaultValue={sectionIds} className="w-full">
      {hasSizeOptions ? (
        <FacetAccordionItem id="size" title="Size">
          <MultiSelectSection
            paramKey={LEASH_FACET_PARAM_KEYS.size}
            options={LEASH_SIZE_OPTIONS}
            selected={selections.sizes}
            counts={counts[LEASH_FACET_PARAM_KEYS.size]}
            state={state}
          />
        </FacetAccordionItem>
      ) : null}
      ...
      <FacetAccordionItem id="condition" title="Condition">
        <MultiSelectSection
          paramKey={LEASH_FACET_PARAM_KEYS.condition}
          options={LEASH_CONDITION_OPTIONS}
          selected={selections.conditions}
          counts={counts[LEASH_FACET_PARAM_KEYS.condition]}
          state={state}
        />
      </FacetAccordionItem>
