...
import { LEASHES_SECTION, leashSizeLabel } from "@/lib/leash-listing-config"
...
const SELLER_LEASHES_PDP_LIMIT = 12

export async function LeashesListingDetailPage(props: ListingDetailPageSharedProps) {
  return renderListingDetailWithGuestFallback(props, renderLeashesListingDetailPage)
}

async function renderLeashesListingDetailPage({
  listingParam,
  prefetchedListing,
  viewerUser,
  ...
  const sellerId = leash.user_id as string
  ...
  const listPriceNum =
    typeof leash.price === "number" ? leash.price : Number.parseFloat(String(leash.price)) || 0
  ...
  const [
    ...
    fetchSimilarPeerListingsForListingPdp(supabase, {
      excludeListingId: leash.id as string,
      section: LEASHES_SECTION,
      priceUsd: listPriceNum,
      facet: { column: "leash_size", value: leash.leash_size as string | null },
    }),
    Promise.all([
  ...
  const sellerLeashes = hydrateHomePeerListingRows((sellerLeashesRes.data ?? []) as Record<string, unknown>[])
  const similarLeashes = hydrateHomePeerListingRows(similarLeashesRaw)

  const sellerLeashIds = (sellerLeashes ?? []).map((f) => f.id)
  const similarLeashIds = similarLeashes.map((r) => String(r.id))
  ...
  const [favoriteRowsRes, acceptedOffer] = await Promise.all([
    user
      ? supabase
          ...
          .select("listing_id")
          .eq("user_id", user.id)
          .in("listing_id", [leash.id, ...sellerLeashIds, ...similarLeashIds])
  ...
  const similarLeashFavoritedIds = similarLeashIds.filter((id) => favoritedIds.has(id))

  const images = orderedListingGalleryImages(leash.listing_images as GalleryImage[] | null)
  ...
  const isOwnListing = user?.id === leash.user_id

  const pickupOffered = leash.local_pickup !== false
  const shippingOffered = !!leash.shipping_available

  const purchaseVisibility = {
    status: String(leash.status ?? ""),
    title: leash.title as string | null | undefined,
    hidden_from_site: leash.hidden_from_site as boolean | null | undefined,
    archived_at: leash.archived_at as string | null | undefined,
  }
  ...
  const modelForSpecs = (leash.model as string | null)?.trim() || null
  const modelPagePath = await resolveListingModelPageHref(supabase, {
    brand: indexBrand,
    brandModelId: (leash.brand_model_id as string | null),
    modelName: modelForSpecs,
  })

  const sizeLabel = leashSizeLabel(leash.leash_size as string | null)

  const listingTitle = capitalizeWords(leash.title as string)

  const publicListPriceUsd = publicListingListPriceUsd(leash.price)
  const compareAtPriceUsd = publicListingCompareAtPriceUsd(
    (leash as { compare_at_price?: string | number | null }).compare_at_price,
    listPriceNum,
  )
  const buyerOffersOn = (leash.buyer_offers_enabled as boolean | null) !== false
  const offerPct = effectiveMinimumOfferPct(leash as { minimum_offer_pct?: number | null })
  ...
  const shippingFlatRate = Math.max(0, Number.parseFloat(String(leash.shipping_price ?? 0)) || 0)
  ...
  const listingLocationLine =
    leash.city && leash.state
      ? `${leash.city}, ${leash.state}`
      : (leash.profiles as { location?: string | null } | null)?.location?.trim() || null
  ...
  const fulfillmentLabels = boardFulfillmentDetailLabels(
    leash.local_pickup,
    leash.shipping_available,
    leash.shipping_price,
    boardShippingCostMode,
  )

  const conditionWords = formatCondition(leash.condition as string | null)
  ...
  const aboutSellerSection = (
    <ListingAboutSellerSection
      profiles={leash.profiles as AboutSellerProfilesProp}
      listingImageFallbacks={[{ listing_images: leash.listing_images }]}
      sellerProfileHref={sellerProfileHref(leash.profiles)}
      messageHrefAuthenticated={`/messages/new?user=${leash.user_id}&listing=${leash.id}`}
      messageHrefLoginRedirect={`/auth/login?redirect=${encodeURIComponent(leashHref)}`}
      isLoggedIn={!!user}
      isOwnListing={isOwnListing}
      isSold={isSold}
      avgRating={sellerAvgRating}
      reviewCount={sellerReviewCount}
      itemsSold={Number((leash.profiles as { sales_count?: number } | null)?.sales_count ?? 0)}
      ...
      <div className="container mx-auto w-full min-w-0 max-w-full px-4 sm:px-6 lg:px-8 lg:!max-w-[min(100%,1320px)] xl:!max-w-[min(100%,1480px)] 2xl:!max-w-[min(100%,1680px)]">
        <div className="mb-3 min-w-0 max-w-full pt-0.5 max-lg:mb-4 lg:mb-8">
          <Breadcrumb>
            <BreadcrumbList className="gap-1 text-[13px] font-normal tracking-wide text-muted-foreground sm:gap-1.5 sm:text-[14px]">
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild className="transition-colors hover:text-foreground">
                  <Link href="/leashes">Leashes</Link>
                </BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator className="text-muted-foreground/70 [&>svg]:stroke-[1.25]" />
              <BreadcrumbItem>
        ...
        <div className="mx-auto grid w-full min-w-0 max-w-full gap-x-8 gap-y-2 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] lg:grid-rows-[auto_auto_auto] lg:[grid-template-areas:'gallery_details'_'about_details'_'similar_similar'] lg:items-start lg:gap-x-12 lg:gap-y-0 xl:gap-x-16">
          ...
          <div className="min-w-0 max-w-full max-lg:order-2 lg:hidden">
            <ListingMobileBuySummary
              listingId={leash.id}
              isLoggedIn={!!user}
              condition={leash.condition as string | null}
              ...
              showScarcity={canPeerPurchase && leash.status === "active"}
              views={listingViews}
            ...
            >
              {canPeerPurchase ? (
                <ListingDetailPeerPurchaseActionsLoader
                  listingId={leash.id}
                  checkoutListingParam={leash.slug ?? leash.id}
                  section="leashes"
                  isLoggedIn={!!user}
          ...
          <div className="min-w-0 space-y-5 max-lg:order-4 lg:[grid-area:details] lg:order-none lg:pt-1">
            <div className="hidden lg:block">
              ...
              {isSold ? (
                ...
                <>
                  <div className="mt-4">
                    ...
                    {listingPurchasable ? (
                      <ListingKlarnaAsLowAs listingId={leash.id} isLoggedIn={!!user} className="mt-2" />
                    ) : null}
                  </div>
              ...
              {!isSold && !isOwnListing && listingPurchasable && leash.status === "active" ? (
            ...
            {!isOwnListing ? (
              <div className="border-b border-neutral-200/90 pb-4 dark:border-neutral-700/70">
                <Link
                  href={`/contact?topic=listing-report&listing=${encodeURIComponent(leash.id)}`}
                ...
                <AccordionContent className="pb-6 pt-0">
                  <div className="text-[16px] leading-[1.65] text-foreground">
                    <TranslateableDescription
                      text={(leash.description as string) || ""}
              ...
              {!isOwnListing && !isSold ? (
                <AccordionItem value="contact" className="border-border/55">
                  ...
                  <AccordionContent className="pb-6 pt-0">
                    <ContactSellerForm
                      listingId={leash.id}
                      listingSlug={leash.slug}
                      sellerId={leash.user_id}
            ...
            {similarLeashes.length > 0 ? (
              <div className="col-span-full min-w-0 max-w-full max-lg:order-5 lg:[grid-area:similar] lg:order-none">
                <section className="mt-10 border-t border-neutral-200/90 pt-8 dark:border-neutral-700/70">
                  <h2 className="mb-8 text-2xl font-bold text-foreground">Similar leashes</h2>
                  <HomeListingScrollRow uniformCardHeights>
                    {similarLeashes.map((row) => (
                      <HomePeerListingScrollTile
                        key={String(row.id)}
                        listing={row as unknown as HomePeerScrollListing}
                        userId={user?.id ?? null}
                        isFavorited={similarLeashFavoritedIds.includes(String(row.id))}
                      />
                    ))}
                  </HomeListingScrollRow>
                </section>
        ...
        {sellerLeashes && sellerLeashes.length > 0 && (
          <section className="mt-16 min-w-0 w-full border-t border-neutral-200/90 pt-12 dark:border-neutral-700/70">
            <h2 className="mb-8 text-2xl font-bold text-foreground">More leashes from this seller</h2>
            <HomeListingScrollRow uniformCardHeights>
              {sellerLeashes.map((item) => (
                <HomePeerListingScrollTile
                  key={item.id}
                  listing={item}
                  userId={user?.id ?? null}
                  isFavorited={sellerLeashFavoritedIds.includes(item.id)}
                />
              ))}
            </HomeListingScrollRow>
          </section>
