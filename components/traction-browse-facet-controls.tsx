...
import type { TractionFilterState } from "@/components/traction-browse-filter-state"
  ...
  return (
    <div className="flex flex-col">
      {options.map((opt) => (
        <CheckRow
          key={opt.value}
          id={`traction-facet-${paramKey}-${opt.value}`}
          label={opt.label}
...
function PriceSection({ state }: { state: TractionFilterState }) {
...
function BrandSection({ state }: { state: TractionFilterState }) {
...
export function TractionBrowseFacetControls({
  state,
  counts = {},
}: {
  state: TractionFilterState
  ...
  return (
    <Accordion type="multiple" defaultValue={sectionIds} className="w-full">
      {hasSizeOptions ? (
        <FacetAccordionItem id="size" title="Type">
          <MultiSelectSection
            paramKey={TRACTION_FACET_PARAM_KEYS.size}
            options={TRACTION_SIZE_OPTIONS}
            selected={selections.sizes}
            counts={counts[TRACTION_FACET_PARAM_KEYS.size]}
            state={state}
          />
        </FacetAccordionItem>
      ) : null}
      ...
      <FacetAccordionItem id="condition" title="Condition">
        <MultiSelectSection
          paramKey={TRACTION_FACET_PARAM_KEYS.condition}
          options={TRACTION_CONDITION_OPTIONS}
          selected={selections.conditions}
          counts={counts[TRACTION_FACET_PARAM_KEYS.condition]}
          state={state}
        />
      </FacetAccordionItem>
