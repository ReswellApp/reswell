...
import type { ApparelFilterState } from "@/components/apparel-browse-filter-state"
  ...
  return (
    <div className="flex flex-col">
      {options.map((opt) => (
        <CheckRow
          key={opt.value}
          id={`apparel-facet-${paramKey}-${opt.value}`}
          label={opt.label}
...
function PriceSection({ state }: { state: ApparelFilterState }) {
...
function BrandSection({ state }: { state: ApparelFilterState }) {
...
export function ApparelBrowseFacetControls({
  state,
  counts = {},
}: {
  state: ApparelFilterState
    ...
    <Accordion type="multiple" defaultValue={sectionIds} className="w-full">
      {hasSizeOptions ? (
        <FacetAccordionItem id="size" title="Size">
          <MultiSelectSection
            paramKey={APPAREL_FACET_PARAM_KEYS.size}
            options={APPAREL_SIZE_OPTIONS}
            selected={selections.sizes}
            counts={counts[APPAREL_FACET_PARAM_KEYS.size]}
            state={state}
          />
        </FacetAccordionItem>
      ) : null}
      ...
      <FacetAccordionItem id="condition" title="Condition">
        <MultiSelectSection
          paramKey={APPAREL_FACET_PARAM_KEYS.condition}
          options={APPAREL_CONDITION_OPTIONS}
          selected={selections.conditions}
          counts={counts[APPAREL_FACET_PARAM_KEYS.condition]}
          state={state}
        />
      </FacetAccordionItem>
