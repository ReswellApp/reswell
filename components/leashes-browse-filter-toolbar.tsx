...
import { LEASHES_BROWSE_DEFAULT_SORT } from "@/lib/leashes-browse-metadata"

export const leashSortOptions = [
  { value: LEASHES_BROWSE_DEFAULT_SORT, label: "Newest" },
...
/** Slim browse toolbar for `/leashes`: keyword search, sort, plus Filter triggers. */
export function LeashesBrowseFilterToolbar({
  resultCount,
  activeFilterCount,
  onOpenMobileFilters,
  ...
  const sort = searchParams.get("sort") ?? LEASHES_BROWSE_DEFAULT_SORT
  ...
  const sortSelect = (
    <Select
      ...
      value={sort}
      onValueChange={(v) =>
        navigate((p) => (v === LEASHES_BROWSE_DEFAULT_SORT ? p.delete("sort") : p.set("sort", v)))
      }
    >
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-3 md:flex-row md:items-center md:gap-3">
        ...
        {resultCount != null ? (
          <span className="order-3 hidden shrink-0 text-sm text-muted-foreground md:order-4 lg:inline">
            {resultCount.toLocaleString()} {resultCount === 1 ? "leash" : "leashes"}
          </span>
        ) : null}
      </div>
    </form>
