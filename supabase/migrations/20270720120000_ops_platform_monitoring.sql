...
COMMENT ON TABLE public.ops_groups IS
  'Deduped platform issues from Vercel logs, Supabase logs, and app client/server capture.';
COMMENT ON TABLE public.ops_signals IS
  'Individual occurrences / log samples linked to an ops_groups row.';
COMMENT ON TABLE public.ops_fix_tickets IS
  'Internal eng/ops fix tickets created from ops_groups.';
COMMENT ON TABLE public.ops_ingest_runs IS
  'Cron/manual ingest run health for Vercel and Supabase log pullers.';
